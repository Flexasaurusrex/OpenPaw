...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("openpaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "openpaw-sandbox-browser-global" },
      agentBrowser: { network: "openpaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("openpaw-sandbox-browser-agent");
  });
