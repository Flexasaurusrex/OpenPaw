...
// Keep integration tests deterministic: never read a real openpaw.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.OPENPAW_SESSION_CACHE_TTL_MS;
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      ...
      process.env.OPENPAW_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
