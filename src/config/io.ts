...
function unsetPathForWrite(
  root: OpenPawConfig,
  pathSegments: string[],
): { changed: boolean; next: OpenPawConfig } {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OpenPawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  async function writeConfigFile(cfg: OpenPawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OpenPawConfig)
        : cfgToWrite;
    ...
    const auditRecordBase = {
      ...
      watchSession:
        typeof deps.env.OPENPAW_WATCH_SESSION === "string" &&
        deps.env.OPENPAW_WATCH_SESSION.trim().length > 0
          ? deps.env.OPENPAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OPENPAW_WATCH_COMMAND === "string" &&
        deps.env.OPENPAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.OPENPAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
