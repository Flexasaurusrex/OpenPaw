...
export function loadOpenPawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "openpaw/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "openpaw/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = getJiti()(candidate.source) as OpenPawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
