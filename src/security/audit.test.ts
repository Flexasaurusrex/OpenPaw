...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: OpenPawConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withStateDir = async (label: string, fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ OPENPAW_STATE_DIR: tmp }, async () => await fn(tmp));
  };

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-security-audit-"));
  });
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevPassword = process.env.OPENPAW_GATEWAY_PASSWORD;
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("openpaw-sbx-browser-old\nopenpaw-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "openpaw-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "openpaw-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("openpaw-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("openpaw-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "openpaw-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "openpaw-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-openpaw.json");
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): OpenPawConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withStateDir("discord-name-based-allowlist", async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.openpaw/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<OpenPawConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: OpenPawConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@openpaw/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@openpaw/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("does not scan plugin code safety findings when deep audit is disabled", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        openpaw: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    beforeEach(() => {
      envSnapshot = captureEnv(["OPENPAW_GATEWAY_TOKEN", "OPENPAW_GATEWAY_PASSWORD"]);
      delete process.env.OPENPAW_GATEWAY_TOKEN;
      delete process.env.OPENPAW_GATEWAY_PASSWORD;
    });
    ...
    const setProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.password !== undefined) {
        process.env.OPENPAW_GATEWAY_PASSWORD = env.password;
      }
    };
