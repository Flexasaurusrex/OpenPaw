...
function isGatewayRemotelyExposed(cfg: OpenPawConfig): boolean {
...
function listKnownNodeCommands(cfg: OpenPawConfig): Set<string> {
  const baseCfg: OpenPawConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
export function collectHooksHardeningFindings(
  ...
  const openpawGatewayToken =
    typeof env.OPENPAW_GATEWAY_TOKEN === "string" && env.OPENPAW_GATEWAY_TOKEN.trim()
      ? env.OPENPAW_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-openpaw-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: OpenPawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: OpenPawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: openpaw-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: OpenPawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: OpenPawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
