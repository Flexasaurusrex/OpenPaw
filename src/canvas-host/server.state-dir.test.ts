...
describe("canvas host state dir defaults", () => {
  it("uses OPENPAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("openpaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("OpenPaw Canvas");
      } finally {
        await handler.close();
      }
    });
