...
function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.OPENPAW_GATEWAY_TOKEN?.trim() || process.env.PAWBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAuthTokenForPort(port: number): string {
  ...
  throw new Error(
    "extension relay requires gateway auth token (set gateway.auth.token or OPENPAW_GATEWAY_TOKEN)",
  );
}

export async function probeAuthenticatedOpenPawRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === OPENPAW_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
