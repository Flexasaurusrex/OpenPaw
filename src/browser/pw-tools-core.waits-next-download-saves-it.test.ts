...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredOpenPawTmpDir: vi.fn(() => "/tmp/openpaw"),
}));
vi.mock("../infra/tmp-openpaw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredOpenPawTmpDir.mockReturnValue("/tmp/openpaw");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredOpenPawTmpDir.mockReturnValue("/tmp/openpaw-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "openpaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredOpenPawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "openpaw-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "openpaw-preferred", "downloads")}${path.sep}`),
    );
  });
