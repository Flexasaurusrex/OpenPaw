    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "openpaw", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-openpaw-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-openpaw-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from OpenPaw.");
      }
