...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "OPENPAW_STATE_DIR",
  "OPENPAW_CONFIG_PATH",
  "OPENPAW_SKIP_BROWSER_CONTROL_SERVER",
  "OPENPAW_SKIP_GMAIL_WATCHER",
  "OPENPAW_SKIP_CANVAS_HOST",
  "OPENPAW_BUNDLED_PLUGINS_DIR",
  "OPENPAW_SKIP_CHANNELS",
  "OPENPAW_SKIP_PROVIDERS",
  "OPENPAW_SKIP_CRON",
  "OPENPAW_TEST_MINIMAL_GATEWAY",
] as const;
...
function applyGatewaySkipEnv() {
  ...
  process.env.OPENPAW_SKIP_PROVIDERS = "1";
  process.env.OPENPAW_SKIP_CRON = "1";
  process.env.OPENPAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.OPENPAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "openpaw-test-no-bundled-extensions")
    : "openpaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "openpaw-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".openpaw-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__openpawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__openpawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["OPENPAW_GATEWAY_TOKEN"]);
