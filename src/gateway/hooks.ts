...
function resolveKnownAgentIds(cfg: OpenPawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-openpaw-token"] === "string"
      ? req.headers["x-openpaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
