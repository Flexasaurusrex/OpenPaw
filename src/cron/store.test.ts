...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses OPENPAW_HOME for tilde expansion", () => {
    vi.stubEnv("OPENPAW_HOME", "/srv/openpaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/openpaw-home", "cron", "jobs.json"));
  });
});
