...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.OPENPAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.OPENPAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set OPENPAW_APNS_PRIVATE_KEY_P8 or OPENPAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading OPENPAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
