...
describe("resolveEffectiveHomeDir", () => {
  it("prefers OPENPAW_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands OPENPAW_HOME when set to ~", () => {
    const env = {
      OPENPAW_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for OPENPAW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { OPENPAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when OPENPAW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ OPENPAW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { OPENPAW_HOME: "/srv/openpaw-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/openpaw-home")}/x`);
  });
