...
vi.mock("./openpaw-root.js", () => ({
  resolveOpenPawPackageRoot: vi.fn(async () => null),
  resolveOpenPawPackageRootSync: vi.fn(() => null),
}));
...
let openpawRoot: typeof import("./openpaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    openpawRoot = await import("./openpaw-root.js");
  });
  ...
  it("uses resolveOpenPawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/openpaw");
    (
      openpawRoot.resolveOpenPawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/openpaw"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "openpaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "openpaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-openpaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/openpaw-bundle");
    (
      openpawRoot.resolveOpenPawPackageRootSync as unknown as ReturnType<typeof vi.fn>
