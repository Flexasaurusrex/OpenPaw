...
let ensureOpenPawCliOnPath: typeof import("./path-env.js").ensureOpenPawCliOnPath;

describe("ensureOpenPawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "OPENPAW_PATH_BOOTSTRAPPED",
    "OPENPAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOpenPawCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling openpaw exists", () => {
    const tmp = abs("/tmp/openpaw-path/case-bundled");
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/openpaw-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/openpaw-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openpaw");
    setDir(tmp);
    setDir(appBinDir);
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/openpaw-path/case-linuxbrew");
