...
import { resolveOpenPawPackageRoot, resolveOpenPawPackageRootSync } from "./openpaw-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveOpenPawPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "openpaw" + dist/control-ui/index.html
