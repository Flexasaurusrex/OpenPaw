...
import { POSIX_OPENPAW_TMP_DIR, resolvePreferredOpenPawTmpDir } from "./tmp-openpaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOpenPawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `openpaw-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredOpenPawTmpDir({
    accessSync,
...
describe("resolvePreferredOpenPawTmpDir", () => {
  it("prefers /tmp/openpaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_OPENPAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/openpaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_OPENPAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/openpaw when /tmp/openpaw is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/openpaw when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/openpaw is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/openpaw is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/openpaw is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
