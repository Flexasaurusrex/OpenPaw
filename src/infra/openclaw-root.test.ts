...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__openpaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "openpaw-root");
...
describe("resolveOpenPawPackageRoot", () => {
  let resolveOpenPawPackageRoot: typeof import("./openpaw-root.js").resolveOpenPawPackageRoot;
  let resolveOpenPawPackageRootSync: typeof import("./openpaw-root.js").resolveOpenPawPackageRootSync;

  beforeAll(async () => {
    ({ resolveOpenPawPackageRoot, resolveOpenPawPackageRootSync } =
      await import("./openpaw-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "openpaw");
    const pkgRoot = path.join(project, "node_modules", "openpaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "openpaw" }));

    expect(resolveOpenPawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "openpaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "openpaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "openpaw" }));

    expect(resolveOpenPawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveOpenPawPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-openpaw package roots", async () => {
    const pkgRoot = fx("not-openpaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-openpaw" }));

    expect(resolveOpenPawPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveOpenPawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveOpenPawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
