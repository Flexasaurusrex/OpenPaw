...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("openpaw-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await tempDirs.make("openpaw-fs-safe-root-");
    const outside = await tempDirs.make("openpaw-fs-safe-outside-");
