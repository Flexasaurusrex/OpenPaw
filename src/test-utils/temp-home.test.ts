...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("openpaw-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.OPENPAW_STATE_DIR).toBe(path.join(tempHome.home, ".openpaw"));
    ...
    expect(process.env.OPENPAW_STATE_DIR).toBe(previousStateDir);
