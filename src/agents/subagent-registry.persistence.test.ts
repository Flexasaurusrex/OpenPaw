...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["OPENPAW_STATE_DIR"]);
  ...
  it("uses isolated temp state when OPENPAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "openpaw-test-state"));
  });
});
