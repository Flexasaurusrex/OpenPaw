...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: OpenPawConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<OpenPawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<OpenPawConfig["agents"]>["list"]>[number]["tools"];
  }): OpenPawConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createOpenPawCodingTools({
      config: cfg,
