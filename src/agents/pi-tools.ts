...
function resolveExecConfig(params: { cfg?: OpenPawConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: OpenPawConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createOpenPawCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createOpenPawReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
