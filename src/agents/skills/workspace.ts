...
function resolveSkillsLimits(config?: OpenPawConfig): ResolvedSkillsLimits {
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: OpenPawConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`openpaw skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
