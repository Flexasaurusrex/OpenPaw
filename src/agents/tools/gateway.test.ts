...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    openpaw: process.env.OPENPAW_GATEWAY_TOKEN,
    pawbot: process.env.PAWBOT_GATEWAY_TOKEN,
  };
  ...
  beforeEach(() => {
    ...
    delete process.env.PAWBOT_GATEWAY_TOKEN;
  });

  afterAll(() => {
    if (envSnapshot.openpaw === undefined) {
      delete process.env.OPENPAW_GATEWAY_TOKEN;
    } else {
      process.env.OPENPAW_GATEWAY_TOKEN = envSnapshot.openpaw;
    }
    if (envSnapshot.pawbot === undefined) {
      delete process.env.PAWBOT_GATEWAY_TOKEN;
    } else {
      process.env.PAWBOT_GATEWAY_TOKEN = envSnapshot.pawbot;
    }
  });
  ...
  it("uses OPENPAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.OPENPAW_GATEWAY_TOKEN = "local-env-token";
    process.env.PAWBOT_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
