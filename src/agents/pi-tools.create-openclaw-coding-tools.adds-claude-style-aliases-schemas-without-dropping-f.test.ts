...
describe("createOpenPawCodingTools", () => {
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-structured-write-"));
    try {
      const tools = createOpenPawCodingTools({ workspaceDir: tmpDir });
      ...
      await writeTool?.execute("tool-structured-write", {
        ...
        content: [
          ...
          { type: "input_text", text: "const root = path.join(process.env.HOME, 'paw');\n" },
        ],
      });

      const written = await fs.readFile(path.join(tmpDir, "structured-write.js"), "utf8");
      expect(written).toBe(
        "const path = require('path');\nconst root = path.join(process.env.HOME, 'paw');\n",
      );
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-structured-edit-"));
    try {
