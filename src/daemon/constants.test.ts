...
describe("resolveGatewayServiceDescription", () => {
  it("prefers explicit description override", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { OPENPAW_PROFILE: "work", OPENPAW_SERVICE_VERSION: "1.0.0" },
        description: "Custom",
      }),
    ).toBe("Custom");
  });
  ...
  it("resolves version from explicit environment map", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { OPENPAW_PROFILE: "work", OPENPAW_SERVICE_VERSION: "local" },
        environment: { OPENPAW_SERVICE_VERSION: "remote" },
      }),
    ).toBe("OpenPaw Gateway (profile: work, vremote)");
  });
});

describe("LEGACY_GATEWAY_SYSTEMD_SERVICE_NAMES", () => {
  it("includes known pre-rebrand gateway unit names", () => {
    expect(LEGACY_GATEWAY_SYSTEMD_SERVICE_NAMES).toContain("pawbot-gateway");
    expect(LEGACY_GATEWAY_SYSTEMD_SERVICE_NAMES).toContain("pawbot-gateway");
  });
});
