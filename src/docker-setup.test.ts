...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    OPENPAW_CONFIG_DIR: join(sandbox.rootDir, "config"),
    OPENPAW_WORKSPACE_DIR: join(sandbox.rootDir, "openpaw"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ...
      OPENPAW_EXTRA_MOUNTS: undefined,
      OPENPAW_HOME_VOLUME: "openpaw-home",
    });
    ...
    expect(envFile).toContain("OPENPAW_HOME_VOLUME=openpaw-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("openpaw-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("openpaw-home:");
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENPAW_CONFIG_DIR: configDir,
      OPENPAW_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("rejects injected multiline OPENPAW_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENPAW_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("OPENPAW_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid OPENPAW_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENPAW_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid OPENPAW_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENPAW_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("OPENPAW_HOME_VOLUME must match");
  });
