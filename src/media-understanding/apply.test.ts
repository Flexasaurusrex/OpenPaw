...
const TEMP_MEDIA_PREFIX = "openpaw-media-";
...
function createGroqAudioConfig(): OpenPawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): OpenPawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): OpenPawConfig {
  return {
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredOpenPawTmpDir();
  ...
  it("auto-detects sherpa for audio when binary and model files are available", async () => {
    ...
    const cfg: OpenPawConfig = { tools: { media: { audio: {} } } };
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        OPENPAW_AGENT_DIR: isolatedAgentDir,
