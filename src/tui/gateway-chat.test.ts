...
describe("resolveGatewayConnection", () => {
  ...
  it("uses OPENPAW_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ OPENPAW_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers OPENPAW_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ OPENPAW_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
