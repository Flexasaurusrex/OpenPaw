...
function createReplyConfig(home: string, streamMode?: "block"): OpenPawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("openpaw-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("OPENPAW_TEST_FAST", "1");
