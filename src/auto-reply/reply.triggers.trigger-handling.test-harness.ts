...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    openpawHome: process.env.OPENPAW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("OPENPAW_HOME", snapshot.openpawHome);
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.OPENPAW_STATE_DIR = join(home, ".openpaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "openpaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".openpaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): OpenPawConfig {
  return {
    agents: {
      defaults: {
