...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OPENPAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("openpaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-oauth-sync-"));
    ...
    process.env.OPENPAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-oauth-nosync-"));
    process.env.OPENPAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside OPENPAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openpaw-oauth-external-"));
    process.env.OPENPAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* OPENPAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENPAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("openpaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
