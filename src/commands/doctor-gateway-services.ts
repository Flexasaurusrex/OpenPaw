...
function resolveGatewayAuthToken(cfg: OpenPawConfig, env: NodeJS.ProcessEnv): string | undefined {
  const configToken = cfg.gateway?.auth?.token?.trim();
  if (configToken) {
    return configToken;
  }
  const envToken = env.OPENPAW_GATEWAY_TOKEN ?? env.PAWBOT_GATEWAY_TOKEN;
