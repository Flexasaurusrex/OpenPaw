...
function scanTelegramAllowFromUsernameEntries(cfg: OpenPawConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: OpenPawConfig): Promise<{
  config: OpenPawConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: OpenPawConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: OpenPawConfig): {
  config: OpenPawConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: OpenPawConfig): {
  config: OpenPawConfig;
  changes: string[];
} {
...
function collectExecSafeBinScopes(cfg: OpenPawConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: OpenPawConfig): ExecSafeBinCoverageHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: OpenPawConfig): {
  config: OpenPawConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: OpenPawConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: OpenPawConfig): {
  config: OpenPawConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("openpaw doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("openpaw doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("openpaw doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("openpaw doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  }
