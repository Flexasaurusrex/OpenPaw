...
type EnvSnapshot = {
  HOME?: string;
  OPENPAW_HOME?: string;
  OPENPAW_STATE_DIR?: string;
  OPENPAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    OPENPAW_HOME: process.env.OPENPAW_HOME,
    OPENPAW_STATE_DIR: process.env.OPENPAW_STATE_DIR,
    OPENPAW_OAUTH_DIR: process.env.OPENPAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: OpenPawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: OpenPawConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "openpaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.OPENPAW_HOME = tempHome;
    process.env.OPENPAW_STATE_DIR = path.join(tempHome, ".openpaw");
    delete process.env.OPENPAW_OAUTH_DIR;
    fs.mkdirSync(process.env.OPENPAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when OPENPAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.OPENPAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OpenPawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
