...
export async function noteSecurityWarnings(cfg: OpenPawConfig) {
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      "  Host exec gating still comes from ~/.openpaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("openpaw approvals get --gateway")}`,
    );
  }
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('openpaw config set session.dmScope "per-channel-peer"') +
