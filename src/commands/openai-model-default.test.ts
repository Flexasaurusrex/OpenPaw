...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: OpenPawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: OpenPawConfig) => { changed: boolean; next: OpenPawConfig };
  defaultModel: string;
  overrideConfig: OpenPawConfig;
  alreadyDefaultConfig: OpenPawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: OpenPawConfig) => config,
      applyDefaultConfig: (config: OpenPawConfig) => config,
      noteAgentModel,
