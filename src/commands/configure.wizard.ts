...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.OPENPAW_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.OPENPAW_GATEWAY_PASSWORD;
