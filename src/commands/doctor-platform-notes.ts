...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["PAWBOT_GATEWAY_TOKEN", await getenv("PAWBOT_GATEWAY_TOKEN")],
    ["PAWBOT_GATEWAY_PASSWORD", await getenv("PAWBOT_GATEWAY_PASSWORD")],
  ...
  const tokenEntries = [
    ["OPENPAW_GATEWAY_TOKEN", await getenv("OPENPAW_GATEWAY_TOKEN")],
  ] as const;
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(([key, value]) => key.startsWith("PAWBOT_") && value?.trim())
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  }
