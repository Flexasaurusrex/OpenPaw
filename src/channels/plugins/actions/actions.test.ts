...
function telegramCfg(): OpenPawConfig {
  return { channels: { telegram: { botToken: "tok" } } } as OpenPawConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: OpenPawConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as OpenPawConfig);
...
function createSignalAccountOverrideCfg(): OpenPawConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as OpenPawConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as OpenPawConfig,
        accountId: undefined,
        params: {
