...
function cfg(accounts?: Record<string, unknown> | null): OpenPawConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as OpenPawConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as OpenPawConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as OpenPawConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as OpenPawConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as OpenPawConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as OpenPawConfig)).toBe("default");
    });
  });
});
