...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  resolveAllowlist: (params: { cfg: OpenPawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: OpenPawConfig; resolved: TResolved }) => OpenPawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: OpenPawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: OpenPawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: OpenPawConfig, policy: ChannelAccessPolicy): OpenPawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: OpenPawConfig, policy: ChannelAccessPolicy): OpenPawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: OpenPawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: OpenPawConfig; resolved: string[] }) => {
      calls.push("apply");
