...
import type { OpenPawConfig, ConfigFileSnapshot } from "../config/types.openpaw.js";
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/openpaw-update-tests";
  let fixtureCount = 0;
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveOpenPawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("openpaw-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveOpenPawPackageRoot).mockClear();
    ...
    readPackageName.mockResolvedValue("openpaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/openpaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "openpaw-gateway" }],
      hints: [],
    });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as OpenPawConfig,
        });
      },
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("openpaw-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ OPENPAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.OPENPAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("openpaw-update-wizard");
    await withEnvAsync({ OPENPAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
