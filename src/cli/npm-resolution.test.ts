...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@openpaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@openpaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@openpaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@openpaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @openpaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@openpaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@openpaw/plugin-alpha@1.2.3",
        installPath: "/tmp/openpaw/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@openpaw/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @openpaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@openpaw/plugin-alpha@latest",
      true,
      "/tmp/openpaw/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@openpaw/plugin-alpha@latest",
      installPath: "/tmp/openpaw/extensions/alpha",
      version: "1.2.3",
