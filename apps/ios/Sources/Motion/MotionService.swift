...
final class MotionService: MotionServicing {
    func activities(params: OpenPawMotionActivityParams) async throws -> OpenPawMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OpenPawMotionActivityEntry], Error>) in
        ...
        return OpenPawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: OpenPawPedometerParams) async throws -> OpenPawPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<OpenPawPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = OpenPawPedometerPayload(
