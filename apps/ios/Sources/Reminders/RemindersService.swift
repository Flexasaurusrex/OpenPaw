...
final class RemindersService: RemindersServicing {
    func list(params: OpenPawRemindersListParams) async throws -> OpenPawRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OpenPawReminderPayload], Error>) in
        ...
        return OpenPawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OpenPawRemindersAddParams) async throws -> OpenPawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OpenPawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OpenPawRemindersAddPayload(reminder: payload)
    }
