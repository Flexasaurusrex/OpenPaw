...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> OpenPawNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> OpenPawNetworkStatusPayload {
        let status: OpenPawNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> OpenPawNetworkStatusPayload {
        OpenPawNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
