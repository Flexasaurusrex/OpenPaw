...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OpenPawContactsSearchParams) async throws -> OpenPawContactsSearchPayload {
        ...
        return OpenPawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OpenPawContactsAddParams) async throws -> OpenPawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OpenPawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OpenPawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OpenPawContactPayload {
        OpenPawContactPayload(
