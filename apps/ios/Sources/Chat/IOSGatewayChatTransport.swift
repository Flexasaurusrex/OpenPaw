...
struct IOSGatewayChatTransport: OpenPawChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.openpaw", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> OpenPawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OpenPawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OpenPawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OpenPawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OpenPawChatAttachmentPayload]) async throws -> OpenPawChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OpenPawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(OpenPawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(OpenPawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenPawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenPawChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenPawAgentEventPayload.self)
                        {
