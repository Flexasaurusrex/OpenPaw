...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.openpaw.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to OpenPaw", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to OpenPaw gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to OpenPaw.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "OpenPawShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "OpenPaw is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "OpenPaw Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("openpaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
            ...
            try await gateway.connect(
                ...
                token: config.token,
                password: config.password,
                connectOptions: makeOptions("pawbot-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
