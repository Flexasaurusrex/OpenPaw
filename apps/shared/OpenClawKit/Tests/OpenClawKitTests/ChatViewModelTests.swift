...
private final class TestChatTransport: @unchecked Sendable, OpenPawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OpenPawChatHistoryPayload]
    private let sessionsResponses: [OpenPawChatSessionsListResponse]

    private let stream: AsyncStream<OpenPawChatTransportEvent>
    private let continuation: AsyncStream<OpenPawChatTransportEvent>.Continuation

    init(
        historyResponses: [OpenPawChatHistoryPayload],
        sessionsResponses: [OpenPawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OpenPawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OpenPawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OpenPawChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [OpenPawChatAttachmentPayload]) async throws -> OpenPawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return OpenPawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OpenPawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OpenPawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OpenPawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = OpenPawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = OpenPawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { OpenPawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = OpenPawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = OpenPawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = OpenPawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { OpenPawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
