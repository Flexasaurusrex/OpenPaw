...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersOpenPawBinary() async throws {
        ...
        let openpawPath = tmp.appendingPathComponent("node_modules/.bin/openpaw")
        try self.makeExec(at: openpawPath)

        let cmd = CommandResolver.openpawCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([openpawPath.path, "gateway"]))
    }
    ...
    @Test func fallsBackToNodeAndScript() async throws {
        ...
        let cmd = CommandResolver.openpawCommand(
            subcommand: "rpc",
            defaults: defaults,
            configRoot: [:],
    ...
    @Test func fallsBackToPnpm() async throws {
        ...
        let cmd = CommandResolver.openpawCommand(subcommand: "rpc", defaults: defaults, configRoot: [:])
    ...
    @Test func configRootLocalOverridesRemoteDefaults() async throws {
        ...
        #expect(cmd.first == openpawPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
