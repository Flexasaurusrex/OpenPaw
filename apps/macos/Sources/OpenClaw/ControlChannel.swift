...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OpenPawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private static func bridgeToProtocolArgs(
        _ value: OpenPawProtocol.AnyCodable?) -> [String: OpenPawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OpenPawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OpenPawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OpenPawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
