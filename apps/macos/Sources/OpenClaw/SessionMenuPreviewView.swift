...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpenPawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpenPawSessionPreviewEntry,
    ...
    private static func cache(payload: OpenPawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpenPawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
    ...
    private static func decodeMessages(_ raw: [OpenPawKit.AnyCodable]) -> [OpenPawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpenPawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpenPawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpenPawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpenPawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpenPawChatMessage) -> String? {
