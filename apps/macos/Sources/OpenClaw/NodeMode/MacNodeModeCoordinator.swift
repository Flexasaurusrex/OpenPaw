...
final class MacNodeModeCoordinator {
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: OpenPawNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [OpenPawCapability.canvas.rawValue, OpenPawCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(OpenPawCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if OpenPawLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(OpenPawCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            OpenPawSystemCommand.which.rawValue,
            OpenPawSystemCommand.run.rawValue,
            OpenPawSystemCommand.execApprovalsGet.rawValue,
            OpenPawSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(OpenPawCapability.camera.rawValue) {
        ...
        if capsSet.contains(OpenPawCapability.location.rawValue) {
