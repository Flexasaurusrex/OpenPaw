...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.openpaw.mac`) even if the shipped
    ...
    {
        if environment["OPENPAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "openpaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "openpaw.nixMode") { return true }
