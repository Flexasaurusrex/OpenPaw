...
- For manual spawns, delivery is resilient:
  - OpenPaw tries direct `agent` delivery first with a stable idempotency key.
...
2. OpenPaw creates or binds a thread to that session target in the active channel.
