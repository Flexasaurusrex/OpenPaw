...
`openpaw security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `openpaw doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
