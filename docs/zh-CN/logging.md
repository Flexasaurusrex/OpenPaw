...
你可以在 `~/.openpaw/openpaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.openpaw/openpaw.json` 的 `logging` 下。
...
- OpenPaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `openpaw plugins enable diagnostics-otel` 启用插件。
...
- `openpaw.tokens`（计数器，属性：`openpaw.token`、`openpaw.channel`、`openpaw.provider`、`openpaw.model`）
- `openpaw.cost.usd`（计数器，属性：`openpaw.channel`、`openpaw.provider`、`openpaw.model`）
- `openpaw.run.duration_ms`（直方图，属性：`openpaw.channel`、`openpaw.provider`、`openpaw.model`）
- `openpaw.context.tokens`（直方图，属性：`openpaw.context`、`openpaw.channel`、`openpaw.provider`、`openpaw.model`）

消息流：

- `openpaw.webhook.received`（计数器，属性：`openpaw.channel`、`openpaw.webhook`）
- `openpaw.webhook.error`（计数器，属性：`openpaw.channel`、`openpaw.webhook`）
- `openpaw.webhook.duration_ms`（直方图，属性：`openpaw.channel`、`openpaw.webhook`）
- `openpaw.message.queued`（计数器，属性：`openpaw.channel`、`openpaw.source`）
- `openpaw.message.processed`（计数器，属性：`openpaw.channel`、`openpaw.outcome`）
- `openpaw.message.duration_ms`（直方图，属性：`openpaw.channel`、`openpaw.outcome`）

队列 + 会话：

- `openpaw.queue.lane.enqueue`（计数器，属性：`openpaw.lane`）
- `openpaw.queue.lane.dequeue`（计数器，属性：`openpaw.lane`）
- `openpaw.queue.depth`（直方图，属性：`openpaw.lane` 或 `openpaw.channel=heartbeat`）
- `openpaw.queue.wait_ms`（直方图，属性：`openpaw.lane`）
- `openpaw.session.state`（计数器，属性：`openpaw.state`、`openpaw.reason`）
- `openpaw.session.stuck`（计数器，属性：`openpaw.state`）
- `openpaw.session.stuck_age_ms`（直方图，属性：`openpaw.state`）
- `openpaw.run.attempt`（计数器，属性：`openpaw.attempt`）
...
- `openpaw.webhook.error`
  - `openpaw.channel`、`openpaw.webhook`、`openpaw.chatId`、`openpaw.error`
- `openpaw.message.processed`
  - `openpaw.channel`、`openpaw.outcome`、`openpaw.chatId`、`openpaw.messageId`、`openpaw.sessionKey`、`openpaw.sessionId`、`openpaw.reason`
- `openpaw.session.stuck`
  - `openpaw.state`、`openpaw.ageMs`、`openpaw.queueDepth`、`openpaw.sessionKey`、`openpaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `openpaw doctor`。
