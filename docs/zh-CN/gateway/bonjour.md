...
OpenPaw 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
2. 在专用区域下发布 `_openpaw-gw._tcp` 的 DNS‑SD 记录
   （示例：`openpaw.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该
   DNS 服务器为客户端（包括 iOS）解析。

OpenPaw 支持任何发现域名；`openpaw.internal.` 只是一个示例。
...
- 从 `~/.openpaw/dns/<domain>.db` 提供你选择的域名服务（示例：`openpaw.internal.`）

从 Tailnet 连接的机器上验证：

```bash
dns-sd -B _openpaw-gw._tcp openpaw.internal.
dig @<TAILNET_IPV4> -p 53 _openpaw-gw._tcp.openpaw.internal PTR +short
```
...
你的发现域名中浏览 `_openpaw-gw._tcp`，无需多播。
...
- 在 `~/.openpaw/openpaw.json` 中设置 `gateway.bind: "tailnet"`。
- 重启 Gateway 网关（或重启 macOS 菜单栏应用）。

## 什么在广播

只有 Gateway 网关广播 `_openpaw-gw._tcp`。

## 服务类型

- `_openpaw-gw._tcp` — Gateway 网关传输信标（被 macOS/iOS/Android 节点使用）。
...
- `transport=gateway`
- `cliPath=<path>`（可选；可运行的 `openpaw` 入口点的绝对路径）
...
- 浏览实例：
  ```bash
  dns-sd -B _openpaw-gw._tcp local.
  ```
- 解析单个实例（替换 `<instance>`）：
  ```bash
  dns-sd -L "<instance>" _openpaw-gw._tcp local.
  ```
...
iOS 节点使用 `NWBrowser` 来发现 `_openpaw-gw._tcp`。
...
- `OPENPAW_DISABLE_BONJOUR=1` 禁用广播（旧版：`OPENPAW_DISABLE_BONJOUR`）。
- `~/.openpaw/openpaw.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `OPENPAW_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（旧版：`OPENPAW_SSH_PORT`）。
- `OPENPAW_TAILNET_DNS` 在 TXT 中发布 MagicDNS 提示（旧版：`OPENPAW_TAILNET_DNS`）。
- `OPENPAW_CLI_PATH` 覆盖广播的 CLI 路径（旧版：`OPENPAW_CLI_PATH`）。
