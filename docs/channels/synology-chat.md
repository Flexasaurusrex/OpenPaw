---
summary: "Synology Chat webhook setup and OpenPaw config"
read_when:
  - Setting up Synology Chat with OpenPaw
...
```bash
openpaw plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your OpenPaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in OpenPaw.
...
- Pairing approvals work with:
  - `openpaw pairing list synology-chat`
  - `openpaw pairing approve synology-chat <CODE>`
...
```bash
openpaw message send --channel synology-chat --target 123456 --text "Hello from OpenPaw"
openpaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
