...
OpenPaw exposes two agent-facing tools for these Markdown files:
...
OpenPaw can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
BM25 + vectors + reranking. Markdown stays the source of truth; OpenPaw shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.openpaw/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.openpaw/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, OpenPaw retries with `qmd query`. If QMD fails or the binary is
  missing, OpenPaw automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- OpenPaw does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - OpenPaw sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index OpenPaw
    uses), run a one-off query with the agent’s XDG dirs.

    OpenPaw’s QMD state lives under your **state dir** (defaults to `~/.openpaw`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    OpenPaw uses:

    ```bash
    # Pick the same state dir OpenPaw uses
    STATE_DIR="${OPENPAW_STATE_DIR:-$HOME/.openpaw}"
...
- When `scope` denies a search, OpenPaw logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, OpenPaw exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.openpaw/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
