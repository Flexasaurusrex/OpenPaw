...
Accessed via `OpenPawPluginApi.runtime` so plugins never import `src/**`.
...
- Add `api.runtime` to `OpenPawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `openpawRuntime: ">=2026.2.0"`).
