---
summary: "Research notes: offline memory system for Paw workspaces (Markdown source-of-truth + derived index)"
read_when:
...
Target: Paw-style workspace (`agents.defaults.workspace`, default `~/.openpaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
- `memory.md` remains “small + core-ish”: the things you want Paw to see every session.
