...
- inject build metadata into Info.plist: `OpenPawBuildTimestamp` (UTC) and `OpenPawGitCommit` (short hash) so the About pane can show build, git, and debug/release channel.
...
- `OpenPawBuildTimestamp`: ISO8601 UTC at package time
- `OpenPawGitCommit`: short git hash (or `unknown` if unavailable)
