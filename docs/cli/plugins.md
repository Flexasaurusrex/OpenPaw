---
summary: "CLI reference for `openpaw plugins` (list, install, uninstall, enable/disable, doctor)"
read_when:
...
openpaw plugins install <npm-spec> --pin
```
...
```bash
openpaw plugins uninstall <id>
openpaw plugins uninstall <id> --dry-run
openpaw plugins uninstall <id> --keep-files
```
...
state dir extensions root (`$OPENPAW_STATE_DIR/extensions/<id>`). Use
...
OpenPaw prints a warning and asks for confirmation before proceeding. Use
