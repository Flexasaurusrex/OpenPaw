...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `openpaw.browserConfigEpoch`) and recommends `openpaw sandbox recreate --browser --all`.
...
```bash
openpaw security audit --json | jq '.summary'
openpaw security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
openpaw security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
