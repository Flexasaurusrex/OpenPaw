...
To reduce top-of-hour load spikes across many gateways, OpenPaw applies a
...
If `delivery` is omitted for isolated jobs, OpenPaw defaults to `announce`.
...
```bash
openpaw cron run <jobId>
openpaw cron run <jobId> --due
```
...
- OpenPaw applies exponential retry backoff for recurring jobs after consecutive errors:
