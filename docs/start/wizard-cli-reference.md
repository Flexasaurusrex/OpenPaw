...
read_when:
  - You need detailed behavior for openpaw onboard
...
This page is the full reference for `openpaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.openpaw/openpaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `openpaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `openpaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `openpaw health`.
    - `openpaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.openpaw/.env` so launchd can read it.
...
- OAuth credentials: `~/.openpaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.openpaw/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`openpaw onboard`](/cli/onboard)
