...
description: "Complete field-by-field reference for ~/.openpaw/openpaw.json"
summary: "Complete reference for every OpenPaw config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.openpaw/openpaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — OpenPaw uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `openpaw doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["openpaw-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `openpaw plugins install @openpaw/mattermost`.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@openpaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `openpaw.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, OpenPaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, OpenPaw assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "openpaw-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.openpaw/sandboxes`
...
noVNC observer access uses VNC auth by default and OpenPaw emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `openpaw-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.openpaw/agents/main/agent",
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:openpaw`   | All built-in tools (excludes provider plugins)                                           |
...
OpenPaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.openpaw/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `OPENPAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `openpaw onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `openpaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `openpaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `openpaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `openpaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `openpaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.openpaw/extensions`, `<workspace>/.openpaw/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or OPENPAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `OPENPAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.openpaw-dev` + port `19001`), `--profile <name>` (uses `~/.openpaw-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.openpaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-openpaw-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `OPENPAW_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or OPENPAW_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__openpaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openpaw__/a2ui/`
...
- Also serves A2UI at `/__openpaw__/a2ui/`.
...
- Hostname defaults to `openpaw`. Override with `OPENPAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.openpaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `openpaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.openpaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${OPENPAW_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.openpaw/credentials/oauth.json`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `openpaw doctor --fix` can strip unknown keys).
