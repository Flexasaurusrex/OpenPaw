...
OpenPaw assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.openpaw`, including `openpaw.json`), treat them as a trusted operator.
...
| -------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full OpenPaw state                               | filesystem perms on `~/.openpaw`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                          | filesystem perms on `~/.openpaw/openpaw.json`                                                   | yes      |
...
`openpaw security audit` includes `config.insecure_or_dangerous_flags` when any
...
OpenPaw includes explicit bypass flags that disable external-content safety wrapping:
...
- Canvas host: `/__openpaw__/canvas/` and `/__openpaw__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_openpaw-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
UI/WebSocket authentication. OpenPaw verifies the identity by resolving the
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only OpenPaw clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default OpenPaw limits these tools
...
4. Re-run `openpaw security audit --deep` and confirm critical findings are resolved.
...
1. Email: [security@openpaw.ai](mailto:security@openpaw.ai)
