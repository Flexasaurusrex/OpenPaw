 ...
 * `openpaw update`. Use it to verify the completion installation flow
...
\`openpaw update\`, \`openpaw doctor\`, and \`openpaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenPaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`openpaw completion --install\` later to enable.`));
    return;
  }
