...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    openpaw_status=$(check_openpaw_auth 2>/dev/null || true)

    claude_expires=0
    openpaw_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        openpaw_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        openpaw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$OPENPAW_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$openpaw_status" \
        --arg be "$openpaw_expires" \
        '{
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$openpaw_status" == EXPIRED* ]] || [[ "$openpaw_status" == MISSING* ]]; then
        echo "OPENPAW_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$openpaw_status" == EXPIRING* ]]; then
