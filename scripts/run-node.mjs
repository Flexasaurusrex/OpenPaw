...
const shouldBuild = (deps) => {
  if (deps.env.OPENPAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OPENPAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[openpaw] ${message}\n`);
};

const runOpenPaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["openpaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
