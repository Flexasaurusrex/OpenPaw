...
MODELS_MODE="${OPENPAW_E2E_MODELS:-${PAWBOT_E2E_MODELS:-both}}" # both|openai|anthropic
INSTALL_TAG="${OPENPAW_INSTALL_TAG:-${PAWBOT_INSTALL_TAG:-latest}}"
E2E_PREVIOUS_VERSION="${OPENPAW_INSTALL_E2E_PREVIOUS:-${PAWBOT_INSTALL_E2E_PREVIOUS:-}}"
SKIP_PREVIOUS="${OPENPAW_INSTALL_E2E_SKIP_PREVIOUS:-${PAWBOT_INSTALL_E2E_SKIP_PREVIOUS:-0}}"
...
if [[ "$INSTALL_TAG" == "beta" ]]; then
  OPENPAW_BETA=1 PAWBOT_BETA=1 curl -fsSL "$INSTALL_URL" | bash
elif [[ "$INSTALL_TAG" != "latest" ]]; then
  OPENPAW_VERSION="$INSTALL_TAG" PAWBOT_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash
else
