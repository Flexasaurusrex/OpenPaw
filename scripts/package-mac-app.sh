...
/usr/libexec/PlistBuddy -c "Set :OpenPawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpenPawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
OPENPAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenPawKit_OpenPawKit.bundle"
if [ -d "$OPENPAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenPawKit_OpenPawKit.bundle"
  cp -R "$OPENPAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenPawKit_OpenPawKit.bundle"
else
  echo "WARN: OpenPawKit resource bundle not found at $OPENPAWKIT_BUNDLE (continuing)" >&2
fi
