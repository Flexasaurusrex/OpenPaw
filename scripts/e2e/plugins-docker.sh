...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  if [ -f dist/index.mjs ]; then
	    OPENPAW_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    OPENPAW_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export OPENPAW_ENTRY

	  home_dir=$(mktemp -d "/tmp/openpaw-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.openpaw/extensions/demo-plugin"

  cat > "$HOME/.openpaw/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.openpaw/extensions/demo-plugin/openpaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$OPENPAW_ENTRY" plugins list --json > /tmp/plugins.json
...
JS
  cat > "$pack_dir/package/openpaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$OPENPAW_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$OPENPAW_ENTRY" plugins list --json > /tmp/plugins2.json
...
JS
  cat > "$dir_plugin/openpaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$OPENPAW_ENTRY" plugins install "$dir_plugin"
	  node "$OPENPAW_ENTRY" plugins list --json > /tmp/plugins3.json
...
JS
  cat > "$file_pack_dir/package/openpaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$OPENPAW_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$OPENPAW_ENTRY" plugins list --json > /tmp/plugins4.json
