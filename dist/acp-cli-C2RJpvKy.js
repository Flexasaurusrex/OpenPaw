...
import { B as theme, S as shortenHomePath, y as resolveUserPath } from "./utils-CLEQvfyB.js";
...
import { t as buildGatewayConnectionDetails } from "./call-DHXpVMRJ.js";
...
import { t as DANGEROUS_ACP_TOOLS } from "./dangerous-tools-B53PqIf6.js";
...
async function createAcpClient(opts = {}) {
	...
	const serverCommand = opts.serverCommand ?? (entryPath ? process.execPath : "openpaw");
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
	...
	if (argv.includes("--token") || argv.includes("--gateway-token")) console.error("Warning: --token can be exposed via process listings. Prefer --token-file or OPENPAW_GATEWAY_TOKEN.");
	if (argv.includes("--password") || argv.includes("--gateway-password")) console.error("Warning: --password can be exposed via process listings. Prefer --password-file or OPENPAW_GATEWAY_PASSWORD.");
...
function registerAcpCli(program) {
	...
	acp.command("client").description("Run an interactive ACP client against the local ACP bridge").option("--cwd <dir>", "Working directory for the ACP session").option("--server <command>", "ACP server command (default: openpaw)").option("--server-args <args...>", "Extra arguments for the ACP server").option("--server-verbose", "Enable verbose logging on the ACP server", false).option("-v, --verbose", "Verbose client logging", false).action(async (opts, command) => {
