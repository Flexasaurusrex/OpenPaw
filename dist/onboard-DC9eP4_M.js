...
import { f as defaultRuntime, p as restoreTerminalState } from "./subsystem-BlU6Lg5x.js";
...
import { a as ensureWorkspaceAndSessions, b as waitForGatewayReachable, c as handleReset, g as resolveControlUiLinks, h as randomToken, n as applyWizardMetadata, t as DEFAULT_WORKSPACE } from "./onboard-helpers-Urhlbb7I.js";
...
import { n as isDeprecatedAuthChoice, r as normalizeLegacyOnboardAuthChoice } from "./auth-choice-legacy-Ca_GuByt.js";
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-Cczbz_hE.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-RjHE2-Zt.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-CqiGvyuQ.js").then((n) => n.i);
	...
	if (!opts.json) runtime.log(`Tip: run \`${formatCliCommand("openpaw configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.openpaw.ai/tools/web`);
}
...
async function runNonInteractiveOnboardingRemote(params) {
	...
	if (opts.json) runtime.log(JSON.stringify(payload, null, 2));
		...
		runtime.log(`Tip: run \`${formatCliCommand("openpaw configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.openpaw.ai/tools/web`);
	}
}
...
async function runNonInteractiveOnboarding(opts, runtime = defaultRuntime) {
	...
	if (snapshot.exists && !snapshot.valid) {
		runtime.error(`Config invalid. Run \`${formatCliCommand("openpaw doctor")}\` to repair it, then re-run onboarding.`);
		runtime.exit(1);
		return;
	}
...
async function onboardCommand(opts, runtime = defaultRuntime) {
	...
	if (process.platform === "win32") runtime.log([
		"Windows detected — OpenPaw runs great on WSL2!",
