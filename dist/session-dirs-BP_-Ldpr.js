import { h as pathExists } from "./utils-CLEQvfyB.js";
...
import { d as channelToNpmTag, l as DEFAULT_PACKAGE_CHANNEL, m as isStableTag, p as isBetaTag, r as compareSemverStrings, s as detectPackageManager$1, u as DEV_BRANCH } from "./channel-account-context-COpC9CMr.js";
...
async function runGatewayUpdate(opts = {}) {
	...
	if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
		...
		const doctorEntry = path.join(gitRoot, "openpaw.mjs");
		if (!await fs.stat(doctorEntry).then(() => true).catch(() => false)) {
			steps.push({
				name: "openpaw doctor entry",
		...
		const doctorStep = await runStep(step("openpaw doctor", [
			process.execPath,
			doctorEntry,
			"doctor",
			"--non-interactive",
			"--fix"
		], gitRoot, { OPENPAW_UPDATE_IN_PROGRESS: "1" }));
