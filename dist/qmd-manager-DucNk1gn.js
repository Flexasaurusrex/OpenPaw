import { s as resolveAgentWorkspaceDir } from "./agent-scope-CG5fO1-r.js";
import { s as resolveStateDir } from "./paths-C12iiEez.js";
import { t as createSubsystemLogger } from "./subsystem-CrsB6DNT.js";
import { b as parseAgentSessionKey } from "./session-key-BlAu1iW4.js";
import "./workspace--8oeKjX4.js";
import "./redact-CFZiIiPD.js";
import "./paths-DIPqyS4l.js";
import { i as listSessionFilesForAgent, n as requireNodeSqlite, r as buildSessionEntry, t as extractKeywords, v as isFileMissingError, y as statRegularFile } from "./query-expansion-Dtas0ERL.js";
	...
	*/
	...
	async ensureMcporterDaemonStarted(mcporter) {
		...
		if (!mcporter.startDaemon) {
			const g = globalThis;
			if (!g.__openpawMcporterColdStartWarned) {
				g.__openpawMcporterColdStartWarned = true;
		...
		const g = globalThis;
		if (!g.__openpawMcporterDaemonStart) g.__openpawMcporterDaemonStart = (async () => {
			try {
				await this.runMcporter(["daemon", "start"], { timeoutMs: 1e4 });
			} catch (err) {
				log.warn(`mcporter daemon start failed: ${String(err)}`);
				delete g.__openpawMcporterDaemonStart;
			}
		})();
		await g.__openpawMcporterDaemonStart;
	}
