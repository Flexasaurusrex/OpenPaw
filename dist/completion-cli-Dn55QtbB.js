...
import { i as routeLogsToStderr } from "./subsystem-BlU6Lg5x.js";
import { t as formatDocsLink } from "./links-Om7vMhDC.js";
import { a as registerCoreCliByName, c as getSubCliEntries, l as registerSubCliByName, r as getCoreCliCommandNames, t as getProgramContext } from "./program-context-CJRdTsUe.js";
...
async function completionCacheExists(shell, binName = "openpaw") {
...
function isCompletionProfileHeader(line) {
	return line.trim() === "# OpenPaw Completion";
}
...
function updateCompletionProfile(content, binName, cachePath, sourceLine) {
	...
	const block = `# OpenPaw Completion\n${sourceLine}`;
...
async function isCompletionInstalled(shell, binName = "openpaw") {
...
* Returns true if profile has `source <(openpaw completion ...)` instead of cached file.
*/
async function usesSlowDynamicCompletion(shell, binName = "openpaw") {
	const profilePath = getShellProfilePath(shell);
	if (!await pathExists(profilePath)) return false;
...
async function installCompletion(shell, yes, binName = "openpaw") {
	const home = process.env.HOME || os.homedir();
	let profilePath = "";
	let sourceLine = "";
