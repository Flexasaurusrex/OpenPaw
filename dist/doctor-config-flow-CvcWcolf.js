import { f as resolveLegacyStateDirs, g as resolveStateDir, m as resolveOAuthDir, p as resolveNewStateDir, t as CONFIG_PATH } from "./paths-5liSMgvV.js";
import { _ as resolveHomeDir, d as isRecord } from "./utils-CLEQvfyB.js";
import { u as resolveDefaultAgentId } from "./agent-scope-DVw9WyO3.js";
import { m as DEFAULT_ACCOUNT_ID, n as DEFAULT_MAIN_KEY, r as buildAgentMainSessionKey, s as normalizeAgentId } from "./session-key-Dx_ARweb.js";
import { t as formatCliCommand } from "./command-format-BMHSLXMa.js";
import { _t as resolveSlackStreamingMode, d as OpenPawSchema, gt as resolveSlackNativeStreaming, ht as resolveDiscordPreviewStreamMode, n as migrateLegacyConfig, o as readConfigFileSnapshot, vt as resolveTelegramPreviewStreamMode } from "./config-BrzzL48f.js";
import { f as parseToolsBySenderTypedKey } from "./dock-NIaOR0r_.js";
import { U as canonicalizeMainSessionAlias, u as saveSessionStore } from "./sessions-CJHPvDu8.js";
import { l as listTelegramAccountIds, u as resolveTelegramAccount } from "./plugins-PixIvqwD.js";
...
import { t as note } from "./note-UN8_Sj2N.js";
...
async function autoMigrateLegacyStateDir(params) {
	...
	if ((params.env ?? process.env).OPENPAW_STATE_DIR?.trim()) return {
		migrated: false,
		skipped: true,
		changes: [],
		warnings: []
	...
	try {
		...
		try {
			...
			try {
				...
				warnings.push(`Rollback failed; set OPENPAW_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`);
...
async function maybeMigrateLegacyConfig() {
	...
	const legacyCandidates = [
		path.join(home, ".pawbot", "pawbot.json"),
...
async function loadAndMaybeMigrateDoctorConfig(params) {
	...
	if (snapshot.legacyIssues.length > 0) {
		...
		} else fixHints.push(`Run "${formatCliCommand("openpaw doctor --fix")}" to apply legacy migrations.`);
	}
	...
	if (normalized.changes.length > 0) {
		...
		else fixHints.push(`Run "${formatCliCommand("openpaw doctor --fix")}" to apply these changes.`);
	}
	...
	if (shouldRepair) {
		...
		if (hits.length > 0) note([`- Telegram allowFrom contains ${hits.length} non-numeric entries (e.g. ${hits[0]?.entry ?? "@"}); Telegram authorization requires numeric sender IDs.`, `- Run "${formatCliCommand("openpaw doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`].join("\n"), "Doctor warnings");
		const discordHits = scanDiscordNumericIdEntries(candidate);
		if (discordHits.length > 0) note([`- Discord allowlists contain ${discordHits.length} numeric entries (e.g. ${discordHits[0]?.path}=${discordHits[0]?.entry}).`, `- Discord IDs must be strings; run "${formatCliCommand("openpaw doctor --fix")}" to convert numeric IDs to quoted strings.`].join("\n"), "Doctor warnings");
		const allowFromScan = maybeRepairOpenPolicyAllowFrom(candidate);
		if (allowFromScan.changes.length > 0) note([...allowFromScan.changes, `- Run "${formatCliCommand("openpaw doctor --fix")}" to add missing allowFrom wildcards.`].join("\n"), "Doctor warnings");
		...
		if (toolsBySenderHits.length > 0) {
			...
			note([
				...
				`- Run "${formatCliCommand("openpaw doctor --fix")}" to migrate legacy keys to typed id: entries.`
			].join("\n"), "Doctor warnings");
		}
		...
		if (safeBinCoverage.length > 0) {
			...
			lines.push(`- Run "${formatCliCommand("openpaw doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`);
			note(lines.join("\n"), "Doctor warnings");
		}
	}
