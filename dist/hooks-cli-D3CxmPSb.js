...
import { B as theme, S as shortenHomePath, t as CONFIG_DIR, y as resolveUserPath } from "./utils-CLEQvfyB.js";
...
import { a as isPathInsideWithRealpath, i as isPathInside, n as MANIFEST_KEY } from "./legacy-names-BiuaFIgp.js";
...
import { u as promptYesNo } from "./tailscale-DNuh1EXL.js";
...
import { c as resolveArchiveKind, i as unscopedPackageName, o as fileExists, s as readJsonFile, t as resolveSafeInstallDir } from "./install-safe-path-DN73ttZx.js";
import { a as resolveInstallModeOptions, c as withExtractedArchiveRoot, i as installPackageDir, l as resolveArchiveSourcePath, n as finalizeNpmSpecArchiveInstall, o as resolveTimedInstallModeOptions, r as installFromNpmSpecArchiveWithInstaller, s as resolveExistingInstallPath, t as validateRegistryNpmSpec } from "./npm-registry-spec-DzId1tYh.js";
import { t as renderTable } from "./table-BgQdk22X.js";
import { a as parseFrontmatter, t as loadWorkspaceHookEntries } from "./workspace-BLa0Bzoq.js";
import { t as buildWorkspaceHookStatus } from "./hooks-status-DWHRA0ab.js";
import { n as resolvePinnedNpmInstallRecordForCli, r as buildPluginStatusReport, t as buildNpmInstallRecordFields } from "./npm-resolution-Dg5PW9_N.js";
...
async function ensureOpenPawHooks(manifest) {
	const hooks = manifest[MANIFEST_KEY]?.hooks;
	if (!Array.isArray(hooks)) throw new Error("package.json missing openpaw.hooks");
	const list = hooks.map((e) => typeof e === "string" ? e.trim() : "").filter(Boolean);
	if (list.length === 0) throw new Error("package.json openpaw.hooks is empty");
	return list;
}
...
async function installHookPackageFromDir(params) {
	...
	for (const entry of hookEntries) {
		...
		if (!isPathInside(params.packageDir, hookDir)) return {
			ok: false,
			error: `openpaw.hooks entry escapes package directory: ${entry}`
		};
		await validateHookDir(hookDir);
		if (!isPathInsideWithRealpath(params.packageDir, hookDir, { requireRealpath: true })) return {
			ok: false,
			error: `openpaw.hooks entry resolves outside package directory: ${entry}`
		};
...
async function installHooksFromNpmSpec(params) {
	...
	return finalizeNpmSpecArchiveInstall(await installFromNpmSpecArchiveWithInstaller({
		tempDirPrefix: "openpaw-hook-pack-",
		spec,
		timeoutMs,
...
function formatHooksList(report, opts) {
	...
	if (hooks.length === 0) return opts.eligible ? `No eligible hooks found. Run \`${formatCliCommand("openpaw hooks list")}\` to see all hooks.` : "No hooks found.";
