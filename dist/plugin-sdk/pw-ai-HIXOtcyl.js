...
import { _ as fetchJson, b as withCdpSocket, d as normalizeCdpWsUrl, g as appendCdpPath, h as withBrowserNavigationPolicy, m as assertBrowserNavigationAllowed, t as getChromeWebSocketUrl, u as formatAriaSnapshot, y as getHeadersWithAuth } from "./chrome-DL68Ap7F.js";
...
function buildTempDownloadPath(fileName) {
	...
	return path.join(resolvePreferredOpenPawTmpDir(), "downloads", `${id}-${safeName}`);
}
...
async function screenshotWithLabelsViaPlaywright(opts) {
	...
	try {
		if (boxes.length > 0) await page.evaluate((labels) => {
			document.querySelectorAll("[data-openpaw-labels]").forEach((el) => el.remove());
...
async function responseBodyViaPlaywright(opts) {
	...
	const resp = await new Promise((resolve, reject) => {
		...
		timer = setTimeout(() => {
			...
			cleanup();
			reject(/* @__PURE__ */ new Error(`Response not found for url pattern "${pattern}". Run '${formatCliCommand("openpaw browser requests")}' to inspect recent network activity.`));
		}, timeout);
	});
	const url = resp.url?.() || "";
