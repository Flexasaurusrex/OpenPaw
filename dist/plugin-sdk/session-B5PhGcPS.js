import { d as readCredsJsonRaw, h as resolveWebCredsPath, l as maybeRestoreCredsFromBackup, m as resolveWebCredsBackupPath, p as resolveDefaultWebAuthDir } from "./accounts-CLTX_mkd.js";
import { A as ensureDir, B as resolveUserPath, Y as danger, lt as toPinoLikeLogger, ot as getChildLogger, tt as success } from "./subsystem-DAkun_KF.js";
import { kt as VERSION } from "./config-CwTZzzhp.js";
...
async function createWaSocket(printQr, verbose, opts = {}) {
	...
	sock.ev.on("connection.update", (update) => {
		try {
			...
			if (connection === "close") {
				if (getStatusCode(lastDisconnect?.error) === DisconnectReason.loggedOut) console.error(danger(`WhatsApp session logged out. Run: ${formatCliCommand("openpaw channels login")}`));
			}
