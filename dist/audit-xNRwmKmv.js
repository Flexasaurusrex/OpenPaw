import { g as resolveStateDir, m as resolveOAuthDir, o as resolveConfigPath } from "./paths-5liSMgvV.js";
import { A as resolveAgentModelFallbackValues, j as resolveAgentModelPrimaryValue, u as resolveDefaultAgentId } from "./agent-scope-DVw9WyO3.js";
import { s as normalizeAgentId } from "./session-key-Dx_ARweb.js";
import { n as runExec } from "./exec-DkdDR1S0.js";
import { t as formatCliCommand } from "./command-format-BMHSLXMa.js";
import { h as GATEWAY_CLIENT_NAMES, m as GATEWAY_CLIENT_MODES } from "./message-channel-DlEv3rvX.js";
import { bt as MAX_INCLUDE_DEPTH, r as createConfigIO, yt as INCLUDE_KEY } from "./config-BrzzL48f.js";
import { i as isPathInside, n as MANIFEST_KEY, r as extensionUsesSkippedScannerPath } from "./legacy-names-BiuaFIgp.js";
import { u as normalizePluginsConfig } from "./manifest-registry-fNuKu-JQ.js";
import { l as normalizeStringEntries } from "./dock-NIaOR0r_.js";
import { C as resolveToolProfilePolicy, M as SANDBOX_BROWSER_SECURITY_HASH_EPOCH, f as resolveSandboxConfigForAgent, l as execDockerRaw, m as resolveSandboxToolPolicyForAgent, u as getBlockedBindReason } from "./sandbox-Bdjajo-M.js";
import { a as resolveGatewayAuth } from "./auth-DiCh-IhS.js";
import { t as resolveGatewayCredentialsFromConfig } from "./credentials-DKKHmYzr.js";
import { a as resolveProfile, i as resolveBrowserConfig, m as resolveBrowserControlAuth } from "./server-context-Ce1QVTUf.js";
import { i as loadWorkspaceSkillEntries } from "./skills-h9Q-iHsk.js";
import { n as formatErrorMessage } from "./errors-DZpIsD32.js";
import { n as listChannelPlugins } from "./plugins-PixIvqwD.js";
import { r as resolveMergedSafeBinProfileFixtures, t as listInterpreterLikeSafeBins } from "./exec-safe-bin-runtime-policy-CGgle7ZQ.js";
import { a as resolveNativeSkillsEnabled, i as resolveNativeCommandsEnabled } from "./commands-C3u9scYW.js";
import { t as GatewayClient } from "./client-8n3vPrA3.js";
import { c as READ_SCOPE, t as buildGatewayConnectionDetails } from "./call-DHXpVMRJ.js";
import { i as readChannelAllowFromStore } from "./pairing-store-DLPx-tMR.js";
import { t as listAgentWorkspaceDirs } from "./workspace-dirs-BJv6AR0c.js";
import { n as isToolAllowedByPolicies, o as pickSandboxToolPolicy } from "./pi-tools.policy-CbGKAXXH.js";
...
import { t as scanDirectoryWithSummary } from "./skill-scanner-CMNzO5-V.js";
import { n as normalizeTelegramAllowFromEntry, t as isNumericTelegramUserId } from "./allow-from-DYZqQ1Ev.js";
...
async function collectChannelSecurityFindings(params) {
	...
	const warnDmPolicy = async (input) => {
		...
		if (dmScope === "main" && isMultiUserDm) findings.push({
			...
			remediation: "Run: " + formatCliCommand("openpaw config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate DM sessions per sender."
		});
	};
	for (const plugin of params.plugins) {
		...
		if (plugin.id === "discord") {
			...
			addDiscordNameBasedEntries({
				...
				source: "~/.openpaw/credentials/discord-allowFrom.json"
			});
...
function collectSecretsInConfigFindings(cfg) {
	...
	if (password && !looksLikeEnvRef(password)) findings.push({
		...
		remediation: "Prefer OPENPAW_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk."
	});
...
function collectHooksHardeningFindings(cfg, env = process.env) {
	...
	const openpawGatewayToken = typeof env.OPENPAW_GATEWAY_TOKEN === "string" && env.OPENPAW_GATEWAY_TOKEN.trim() ? env.OPENPAW_GATEWAY_TOKEN.trim() : null;
	const gatewayToken = gatewayAuth.mode === "token" && typeof gatewayAuth.token === "string" && gatewayAuth.token.trim() ? gatewayAuth.token.trim() : openpawGatewayToken ? openpawGatewayToken : null;
...
function collectGatewayHttpSessionKeyOverrideFindings(cfg) {
	...
	findings.push({
		...
		detail: `${enabledEndpoints.join(", ")} accept x-openpaw-session-key for per-request session routing. Treat API credential holders as trusted principals.`
	});
	return findings;
}
...
function collectSandboxDangerousConfigFindings(cfg) {
	...
	if (browserExposurePaths.length > 0) findings.push({
		...
		remediation: "Set sandbox.browser.network to a dedicated bridge network (recommended default: openpaw-sandbox-browser), or set sandbox.browser.cdpSourceRange (for example 172.21.0.1/32) to restrict container-edge CDP ingress."
	});
	return findings;
}
...
async function listSandboxBrowserContainers(execDockerRawFn) {
	try {
		const result = await execDockerRawFn([
			...
			"-a",
			"--filter",
			"label=openpaw.sandboxBrowser=1",
			"--format",
			"{{.Names}}"
		], { allowFailure: true });
...
async function readSandboxBrowserHashLabels(params) {
	try {
		const result = await params.execDockerRawFn([
			"inspect",
			"-f",
			"{{ index .Config.Labels \"openpaw.configHash\" }}	{{ index .Config.Labels \"openpaw.browserConfigEpoch\" }}",
...
async function collectSandboxBrowserHashLabelFindings(params) {
	...
	if (missingHash.length > 0) findings.push({
		...
		remediation: `${formatCliCommand("openpaw sandbox recreate --browser --all")} (add --force to skip prompt).`
	});
	if (staleEpoch.length > 0) findings.push({
		...
		detail: `Containers: ${staleEpoch.join(", ")}. Expected openpaw.browserConfigEpoch=${SANDBOX_BROWSER_SECURITY_HASH_EPOCH}.`,
		remediation: `${formatCliCommand("openpaw sandbox recreate --browser --all")} (add --force to skip prompt).`
	});
	if (nonLoopbackPublished.length > 0) findings.push({
		...
		remediation: `${formatCliCommand("openpaw sandbox recreate --browser --all")} (add --force to skip prompt), then verify published ports are bound to 127.0.0.1.`
	});
	return findings;
}
async function collectPluginsTrustFindings(params) {
	...
	if (pluginDirs.length > 0) {
		...
		if (enabledExtensionPluginIds.length > 0) {
			...
			for (const context of contexts) {
				...
				const broadPolicy = isToolAllowedByPolicies("__openpaw_plugin_probe__", resolveToolPolicies({
					cfg: params.cfg,
					agentTools: context.tools,
					sandboxMode,
	...
	if (npmPluginInstalls.length > 0) {
		...
		if (pluginVersionDrift.length > 0) findings.push({
			...
			remediation: "Run `openpaw plugins update --all` (or reinstall affected plugins) to refresh install metadata."
		});
	}
	...
	if (npmHookInstalls.length > 0) {
		...
		if (hookVersionDrift.length > 0) findings.push({
			...
			remediation: "Run `openpaw hooks update --all` (or reinstall affected hooks) to refresh install metadata."
		});
	}
	return findings;
}
...
async function collectPluginsCodeSafetyFindings(params) {
	...
	const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
		...
		onReadError: (err) => {
			findings.push({
				...
				remediation: "Check file permissions and plugin layout, then rerun `openpaw security audit --deep`."
			});
		}
	});
	for (const pluginName of pluginDirs) {
		...
		if (escapedEntries.length > 0) findings.push({
			...
			remediation: "Update the plugin manifest so all openpaw.extensions entries stay inside the plugin directory."
		});
		...
		if (summary.critical > 0) {
			...
			findings.push({
				...
				remediation: "Review the plugin source code carefully before use. If untrusted, remove the plugin from your OpenPaw extensions state directory."
			});
...
async function collectInstalledSkillsCodeSafetyFindings(params) {
	...
	for (const workspaceDir of workspaceDirs) {
		...
		for (const entry of entries) {
			if (entry.skill.source === "openpaw-bundled") continue;
			...
			const summary = await scanDirectoryWithSummary(skillDir).catch((err) => {
				findings.push({
					...
					remediation: "Check file permissions and skill layout, then rerun `openpaw security audit --deep`."
				});
				return null;
			});
			if (!summary) continue;
