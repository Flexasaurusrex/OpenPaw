import { f as normalizeSkillFilter } from "./agent-scope-9aJRv6zx.js";
import { C as CONFIG_DIR, L as resolveUserPath, t as createSubsystemLogger } from "./subsystem-DWLvTobT.js";
import { h as resolveOpenPawPackageRootSync } from "./workspace-DlPWdeFR.js";
import { rt as isDangerousHostEnvVarName } from "./model-auth-HfI5mzXM.js";
import { c as resolveMemorySlotDecision, o as normalizePluginsConfig, s as resolveEnableState, t as loadPluginManifestRegistry } from "./manifest-registry-Bp6yP7WJ.js";
import { a as resolveOpenPawManifestBlock, c as resolveOpenPawManifestRequires, d as hasBinary, f as isConfigPathTruthyWithDefaults, i as parseOpenPawManifestInstallBase, l as parseFrontmatterBlock, n as normalizeStringList, o as resolveOpenPawManifestInstall, r as parseFrontmatterBool, s as resolveOpenPawManifestOs, t as getFrontmatterString, u as evaluateRuntimeEligibility } from "./frontmatter-Bm1Lr5Td.js";
...
function parseInstallSpec(input) {
	const parsed = parseOpenPawManifestInstallBase(input, [
		"brew",
		"node",
		"go",
		"uv",
...
function resolveOpenPawMetadata(frontmatter) {
	const metadataObj = resolveOpenPawManifestBlock({ frontmatter });
	if (!metadataObj) return;
	const requires = resolveOpenPawManifestRequires(metadataObj);
	const install = resolveOpenPawManifestInstall(metadataObj, parseInstallSpec);
	const osRaw = resolveOpenPawManifestOs(metadataObj);
	return {
...
const BLOCKED_ENV_VAR_PATTERNS = [
	...
	/^OPENPAW_GATEWAY_(TOKEN|PASSWORD)$/i,
...
function resolveBundledSkillsDir(opts = {}) {
	...
	try {
		...
		const packageRoot = resolveOpenPawPackageRootSync({
...
function resolveWorkspaceSkillPromptState(workspaceDir, opts) {
	...
	return {
		...
		prompt: [
			remoteNote,
			truncated ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`openpaw skills check\` to audit.` : "",
