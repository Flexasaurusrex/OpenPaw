import { E as runExec, _ as DEFAULT_SOUL_FILENAME, b as ensureAgentWorkspace, f as DEFAULT_AGENTS_FILENAME, g as DEFAULT_IDENTITY_FILENAME, h as DEFAULT_HEARTBEAT_FILENAME, m as DEFAULT_BOOTSTRAP_FILENAME, n as resolveAgentConfig, p as DEFAULT_AGENT_WORKSPACE_DIR, u as resolveSessionAgentId, v as DEFAULT_TOOLS_FILENAME, y as DEFAULT_USER_FILENAME } from "./agent-scope-DDpD-TG0.js";
import { i as resolveGatewayPort, t as STATE_DIR } from "./paths-C0uiCw4Q.js";
import { L as resolveUserPath, W as truncateUtf16Safe, ct as resolvePreferredOpenPawTmpDir, r as defaultRuntime, s as CHANNEL_IDS, t as createSubsystemLogger } from "./subsystem-8Ku6x7B-.js";
import { l as resolveAgentIdFromSessionKey, s as normalizeAgentId } from "./session-key-BlAu1iW4.js";
import { n as parseBooleanValue, r as formatCliCommand } from "./env-DiD38AJD.js";
import { t as generateSecureToken } from "./secure-random-xpZo1jCG.js";
import { c as SafeOpenError, i as getImageMetadata, l as openFileWithinRoot, n as buildImageResizeSideGrid, s as resizeToJpeg, t as IMAGE_REDUCE_QUALITY_STEPS } from "./image-ops-B46KQegA.js";
import { n as loadConfig, o as writeConfigFile, t as createConfigIO } from "./config-3ZvLrn4o.js";
import { A as DEFAULT_BROWSER_EVALUATE_ENABLED, D as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS, E as DEFAULT_AI_SNAPSHOT_EFFICIENT_DEPTH, M as DEFAULT_OPENPAW_BROWSER_ENABLED, N as DEFAULT_OPENPAW_BROWSER_PROFILE_NAME, O as DEFAULT_AI_SNAPSHOT_MAX_CHARS, S as stopChromeExtensionRelayServer, _ as fetchJson, a as resolveOpenPawUserDataDir, c as captureScreenshot, d as normalizeCdpWsUrl, f as snapshotAria, g as appendCdpPath, h as withBrowserNavigationPolicy, i as launchOpenPawChrome, j as DEFAULT_OPENPAW_BROWSER_COLOR, k as DEFAULT_BROWSER_DEFAULT_PROFILE_NAME, l as createTargetViaCdp, m as assertBrowserNavigationAllowed, n as isChromeCdpReady, o as stopOpenPawChrome, p as InvalidBrowserNavigationUrlError, r as isChromeReachable, s as resolveBrowserExecutableForPlatform, v as fetchOk, w as isLoopbackHost, x as ensureChromeExtensionRelayServer } from "./chrome-kqxrOHed.js";
import { t as SsrFBlockedError } from "./ssrf-D_av5wos.js";
import { a as syncSkillsToWorkspace, l as resolveSandboxInputPath, m as sanitizeEnvVars, u as resolveSandboxPath } from "./skills-vF-UFSc7.js";
import { n as formatErrorMessage, t as extractErrorCode } from "./errors-BA3agtNw.js";
import { r as saveMediaBuffer, t as ensureMediaDir } from "./store-9ma4HK4z.js";
import { B as resolveAgentMainSessionKey, G as acquireSessionWriteLock, z as canonicalizeMainSessionAlias } from "./sessions-B87EOf_z.js";
import { t as sanitizeContentBlocksImages } from "./tool-images-D2hZ3OHe.js";
...
const DEFAULT_SANDBOX_IMAGE = "openpaw-sandbox:bookworm-slim";
const DEFAULT_SANDBOX_CONTAINER_PREFIX = "openpaw-sbx-";
...
const DEFAULT_SANDBOX_BROWSER_IMAGE = "openpaw-sandbox-browser:bookworm-slim";
const SANDBOX_BROWSER_SECURITY_HASH_EPOCH = "2026-02-21-novnc-auth-default";
const DEFAULT_SANDBOX_BROWSER_PREFIX = "openpaw-sbx-browser-";
const DEFAULT_SANDBOX_BROWSER_NETWORK = "openpaw-sandbox-browser";
...
function buildCoreToolGroupMap() {
	...
	const openpawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInOpenPawGroup).map((tool) => tool.id);
	return {
		"group:openpaw": openpawTools,
...
function readGatewayTokenEnv(env, includeLegacyEnv) {
	const primary = trimToUndefined(env.OPENPAW_GATEWAY_TOKEN);
	if (primary) return primary;
	if (!includeLegacyEnv) return;
	return trimToUndefined(env.PAWBOT_GATEWAY_TOKEN);
}
function readGatewayPasswordEnv(env, includeLegacyEnv) {
	const primary = trimToUndefined(env.OPENPAW_GATEWAY_PASSWORD);
	if (primary) return primary;
	if (!includeLegacyEnv) return;
	return trimToUndefined(env.PAWBOT_GATEWAY_PASSWORD);
}
...
async function loadPwAiModule(mode) {
	try {
		return await import("./pw-ai-BAuKQIW3.js");
	} catch (err) {
...
const DEFAULT_BROWSER_TMP_DIR = resolvePreferredOpenPawTmpDir();
			...
			run: async ({ cdpUrl, tab, pw }) => {
				...
				switch (kind) {
					...
					case "wait": {
						...
						if (fn && !evaluateEnabled) return jsonError(res, 403, ["wait --fn is disabled by config (browser.evaluateEnabled=false).", "Docs: /gateway/configuration#browser-openpaw-managed-browser"].join("\n"));
					...
					case "evaluate": {
						if (!evaluateEnabled) return jsonError(res, 403, ["act:evaluate is disabled by config (browser.evaluateEnabled=false).", "Docs: /gateway/configuration#browser-openpaw-managed-browser"].join("\n"));
...
function normalizeHexColor(raw) {
	...
	if (!value) return DEFAULT_OPENPAW_BROWSER_COLOR;
	const normalized = value.startsWith("#") ? value : `#${value}`;
	if (!/^#[0-9a-fA-F]{6}$/.test(normalized)) return DEFAULT_OPENPAW_BROWSER_COLOR;
...
function ensureDefaultProfile(profiles, defaultColor, legacyCdpPort, derivedDefaultCdpPort) {
	...
	if (!result[DEFAULT_OPENPAW_BROWSER_PROFILE_NAME]) result[DEFAULT_OPENPAW_BROWSER_PROFILE_NAME] = {
...
function resolveBrowserConfig(cfg, rootConfig) {
	...
	const defaultProfile = defaultProfileFromConfig ?? (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME] ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME : DEFAULT_OPENPAW_BROWSER_PROFILE_NAME);
...
function createProfileContext(opts, profile) {
	...
	const ensureBrowserAvailable = async () => {
		...
		if (isExtension) {
			...
			throw new Error(`Chrome extension relay is running, but no tab is connected. Click the OpenPaw Chrome extension icon on a tab to attach it (profile "${profile.name}").`);
		}
		if (!httpReachable) {
			...
			attachRunning(await launchOpenPawChrome(current.resolved, profile));
			return;
		}
		if (await isReachable()) return;
		if (!profileState.running) throw new Error(`Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by openpaw. Run action=reset-profile profile=${profile.name} to kill the process.`);
	...
	const ensureTabAvailable = async (targetId) => {
		...
		if ((await listTabs()).length === 0) {
			if (profile.driver === "extension") throw new Error(`tab not found (no attached Chrome tabs for profile "${profile.name}"). Click the OpenPaw Browser Relay toolbar icon on the tab you want to control (badge ON).`);
	...
	const resetProfile = async () => {
		...
		if (await isHttpReachable(300) && !profileState.running) try {
			await (await import("./pw-ai-BAuKQIW3.js")).closePlaywrightBrowserConnection();
		} catch {}
...
function isAuthorizedBrowserRequest(req, auth) {
	...
	if (auth.password) {
		const passwordHeader = firstHeaderValue(req.headers["x-openpaw-password"]).trim();
...
function buildSandboxCreateArgs(params) {
	...
	if (params.configHash) args.push("--label", `openpaw.configHash=${params.configHash}`);
...
async function readContainerConfigHash(containerName) {
	return await readDockerContainerLabel(containerName, "openpaw.configHash");
}
function formatSandboxRecreateHint(params) {
	if (params.scope === "session") return formatCliCommand(`openpaw sandbox recreate --session ${params.sessionKey}`);
	if (params.scope === "agent") return formatCliCommand(`openpaw sandbox recreate --agent ${resolveSandboxAgentId(params.sessionKey) ?? "main"}`);
...
const NOVNC_PASSWORD_ENV_KEY = "OPENPAW_BROWSER_NOVNC_PASSWORD";
...
const CDP_SOURCE_RANGE_ENV_KEY = "OPENPAW_BROWSER_CDP_SOURCE_RANGE";
...
function buildSandboxBrowserResolvedConfig(params) {
	return {
		...
		extraArgs: [],
		profiles: { [DEFAULT_OPENPAW_BROWSER_PROFILE_NAME]: {
...
async function ensureSandboxBrowser(params) {
	...
	if (hasContainer) {
		...
		currentHash = await readDockerContainerLabel(containerName, "openpaw.configHash");
		hashMismatch = !currentHash || currentHash !== expectedHash;
		if (!currentHash) {
		...
		if (hashMismatch) {
			...
			if (running && (typeof lastUsedAtMs !== "number" || now - lastUsedAtMs < HOT_BROWSER_WINDOW_MS)) {
				const hint = (() => {
					if (params.cfg.scope === "session") return `openpaw sandbox recreate --browser --session ${params.scopeKey}`;
					if (params.cfg.scope === "agent") return `openpaw sandbox recreate --browser --agent ${resolveSandboxAgentId(params.scopeKey) ?? "main"}`;
					return "openpaw sandbox recreate --browser --all";
				})();
	...
	if (!hasContainer) {
		...
		const args = buildSandboxCreateArgs({
			...
			labels: {
				...
				"openpaw.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH
			},
			configHash: expectedHash,
			includeBinds: false
		});
	...
	const existingProfile = existing ? resolveProfile(existing.bridge.state.resolved, DEFAULT_OPENPAW_BROWSER_PROFILE_NAME) : null;
...
function formatSandboxToolPolicyBlockedMessage(params) {
	...
	lines.push(`- See: ${formatCliCommand(`openpaw sandbox explain --session ${runtime.sessionKey}`)}`);
	return lines.join("\n");
}
