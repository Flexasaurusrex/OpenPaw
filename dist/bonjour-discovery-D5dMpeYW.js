...
import { t as isTailnetIPv4 } from "./tailnet-CfnDL3PT.js";
...
const GATEWAY_SERVICE_TYPE = "_openpaw-gw._tcp";
...
function parseDnsSdBrowse(stdout) {
	...
	for (const raw of stdout.split("\n")) {
		...
		const match = line.match(/_openpaw-gw\._tcp\.?\s+(.+)$/);
...
async function discoverWideAreaViaTailnetDns(domain, timeoutMs, run) {
	...
	for (const ptr of ptrs) {
		...
		if (!ptrName) continue;
		const instanceName = ptrName.replace(/\.?_openpaw-gw\._tcp\..*$/, "");
		const srv = await run([
			"dig",
			"+short",
			"+time=1",
