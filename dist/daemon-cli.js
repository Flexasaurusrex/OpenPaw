...
function resolveRawHomeDir(env, homedir) {
	const explicitHome = normalize(env.OPENPAW_HOME);
	if (explicitHome) {
...
/** Build a homedir thunk that respects OPENPAW_HOME for the given env. */
...
function resolveStateDir(env = process.env, homedir = envHomedir(env)) {
	...
	const override = env.OPENPAW_STATE_DIR?.trim() || env.PAWBOT_STATE_DIR?.trim();
...
function resolveCanonicalConfigPath(env = process.env, stateDir = resolveStateDir(env, envHomedir(env))) {
	const override = env.OPENPAW_CONFIG_PATH?.trim() || env.PAWBOT_CONFIG_PATH?.trim();
...
function resolveDefaultConfigCandidates(env = process.env, homedir = envHomedir(env)) {
	const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
	const explicit = env.OPENPAW_CONFIG_PATH?.trim() || env.PAWBOT_CONFIG_PATH?.trim();
	if (explicit) return [resolveUserPath$1(explicit, env, effectiveHomedir)];
	const candidates = [];
	const openpawStateDir = env.OPENPAW_STATE_DIR?.trim() || env.PAWBOT_STATE_DIR?.trim();
	if (openpawStateDir) {
		const resolved = resolveUserPath$1(openpawStateDir, env, effectiveHomedir);
...
function resolveGatewayPort(cfg, env = process.env) {
	const envRaw = env.OPENPAW_GATEWAY_PORT?.trim() || env.PAWBOT_GATEWAY_PORT?.trim();
	if (envRaw) {
...
//#endregion
//#region src/infra/tmp-openpaw-dir.ts
const POSIX_OPENPAW_TMP_DIR = "/tmp/openpaw";
function isNodeErrorWithCode(err, code) {
	return typeof err === "object" && err !== null && "code" in err && err.code === code;
}
function resolvePreferredOpenPawTmpDir(options = {}) {
	...
	const fallback = () => {
		const base = tmpdir();
		const suffix = uid === void 0 ? "openpaw" : `openpaw-${uid}`;
		return path.join(base, suffix);
	};
	try {
		const preferred = lstatSync(POSIX_OPENPAW_TMP_DIR);
		if (!preferred.isDirectory() || preferred.isSymbolicLink()) return fallback();
		accessSync(POSIX_OPENPAW_TMP_DIR, fs.constants.W_OK | fs.constants.X_OK);
		if (!isSecureDirForUser(preferred)) return fallback();
		return POSIX_OPENPAW_TMP_DIR;
	} catch (err) {
		if (!isNodeErrorWithCode(err, "ENOENT")) return fallback();
	}
	try {
		accessSync("/tmp", fs.constants.W_OK | fs.constants.X_OK);
		mkdirSync(POSIX_OPENPAW_TMP_DIR, {
			recursive: true,
			mode: 448
		});
		try {
...
function resolveEnvLogLevelOverride() {
	const raw = process.env.OPENPAW_LOG_LEVEL;
	...
	if (loggingState.invalidEnvLogLevelValue !== trimmed) {
		...
		process.stderr.write(`[openpaw] Ignoring invalid OPENPAW_LOG_LEVEL="${trimmed}" (allowed: ${ALLOWED_LOG_LEVELS.join("|")}).\n`);
	}
}
...
const DEFAULT_LOG_DIR = resolvePreferredOpenPawTmpDir();
const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "openpaw.log");
...
function resolveSettings() {
	...
	const defaultLevel = process.env.VITEST === "true" && process.env.OPENPAW_TEST_FILE_LOG !== "1" ? "silent" : "info";
		...
		try {
			...
			if (nextBytes > settings.maxFileBytes) {
				if (!warnedAboutSizeCap) {
					...
					process.stderr.write(`[openpaw] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`);
				}
				return;
			}
...
function resolveConfigDir(env = process.env, homedir = os.homedir) {
	...
	const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".openpaw");
	try {
		if (fs.existsSync(newDir)) return newDir;
	} catch {}
	return newDir;
...
function resolveHomeDisplayPrefix() {
	...
	if (!home) return;
	if (process.env.OPENPAW_HOME?.trim()) return {
		home,
		prefix: "$OPENPAW_HOME"
	};
	return {
...
const DOCS_ROOT = "https://docs.openpaw.ai";
...
const DEFAULT_CLI_NAME = "openpaw";
const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE$1 = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(openpaw)\b/;
...
function shouldEmitRuntimeLog(env = process.env) {
	...
	if (env.OPENPAW_TEST_RUNTIME_LOG === "1") return true;
...
async function buildGatewayInstallPlan(params) {
	...
	const serviceEnvironment = buildServiceEnvironment({
		...
		port: params.port,
		token: params.token,
		launchdLabel: process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(params.env.OPENPAW_PROFILE) : void 0
	});
...
function normalizeConsoleLevel(level) {
	...
	if (!level && process.env.VITEST === "true" && process.env.OPENPAW_TEST_CONSOLE !== "1") return "silent";
...
function resolveDefaultAgentWorkspaceDir(env = process.env, homedir = os.homedir) {
	...
	if (profile && profile.toLowerCase() !== "default") return path.join(home, ".openpaw", `workspace-${profile}`);
	return path.join(home, ".openpaw", "workspace");
}
...
const HELD_LOCKS$1 = resolveProcessScopedMap(Symbol.for("openpaw.fileLockHeldLocks"));
...
const PROJECT_NAME = "openpaw";
const MANIFEST_KEY = PROJECT_NAME;

//#endregion
//#region src/plugins/manifest.ts
const PLUGIN_MANIFEST_FILENAME = "openpaw.plugin.json";
...
function discoverOpenPawPlugins(params) {
	const candidates = [];
	const diagnostics = [];
	const seen = /* @__PURE__ */ new Set();
...
function shouldUseManifestCache(env) {
	if (env.OPENPAW_DISABLE_PLUGIN_MANIFEST_CACHE?.trim()) return false;
...
const OpenPawSchema = z.object({
	...
	browser: z.object({
		...
		profiles: z.record(z.string().regex(/^[a-z0-9-]+$/, "Profile names must be alphanumeric with hyphens only"), z.object({
			...
			cdpUrl: z.string().optional(),
			driver: z.union([z.literal("paw"), z.literal("extension")]).optional(),
...
function parseOpenPawVersion(raw) {
	if (!raw) return null;
...
function compareOpenPawVersions(a, b) {
...
function warnIfConfigFromFuture(cfg, logger) {
	...
	if (cmp === null) return;
	if (cmp < 0) logger.warn(`Config was last written by a newer OpenPaw (${touched}); current version is ${VERSION}.`);
}
...
function createConfigIO(overrides = {}) {
	...
	async function writeConfigFile(cfg, options = {}) {
		...
		const logConfigOverwrite = () => {
			...
			const shouldLogInVitest = deps.env.OPENPAW_TEST_CONFIG_OVERWRITE_LOG === "1";
		...
		const logConfigWriteAnomalies = () => {
			...
			const isVitest = deps.env.VITEST === "true";
			const shouldLogInVitest = deps.env.OPENPAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
			if (isVitest && !shouldLogInVitest) return;
		...
		const auditRecordBase = {
			...
			watchMode: deps.env.OPENPAW_WATCH_MODE === "1",
			watchSession: typeof deps.env.OPENPAW_WATCH_SESSION === "string" && deps.env.OPENPAW_WATCH_SESSION.trim().length > 0 ? deps.env.OPENPAW_WATCH_SESSION.trim() : null,
			watchCommand: typeof deps.env.OPENPAW_WATCH_COMMAND === "string" && deps.env.OPENPAW_WATCH_COMMAND.trim().length > 0 ? deps.env.OPENPAW_WATCH_COMMAND.trim() : null,
			existsBefore: snapshot.exists,
...
function shouldUseConfigCache(env) {
	if (env.OPENPAW_DISABLE_CONFIG_CACHE?.trim()) return false;
...
const CLEANUP_STATE_KEY = Symbol.for("openpaw.sessionWriteLockCleanupState");
const HELD_LOCKS_KEY = Symbol.for("openpaw.sessionWriteLockHeldLocks");
const WATCHDOG_STATE_KEY = Symbol.for("openpaw.sessionWriteLockWatchdogState");
...
/**
* Strips OpenPaw-injected inbound metadata blocks from a user-role message
...
function readGatewayTokenEnv(env, includeLegacyEnv) {
	const primary = trimToUndefined$1(env.OPENPAW_GATEWAY_TOKEN);
	if (primary) return primary;
	if (!includeLegacyEnv) return;
	return trimToUndefined$1(env.PAWBOT_GATEWAY_TOKEN);
}
function readGatewayPasswordEnv(env, includeLegacyEnv) {
	const primary = trimToUndefined$1(env.OPENPAW_GATEWAY_PASSWORD);
	if (primary) return primary;
	if (!includeLegacyEnv) return;
	return trimToUndefined$1(env.PAWBOT_GATEWAY_PASSWORD);
}
...
async function installLaunchAgent({ env, stdout, programArguments, workingDirectory, environment, description }) {
	...
	if (boot.code !== 0) {
		...
		if (isUnsupportedGuiDomain(detail)) throw new Error([
			...
			"Fix: sign in to the macOS desktop as the target user and rerun `openpaw gateway install --force`.",
			"Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.openpaw.ai/gateway"
		].join("\n"));
...
function buildSystemdUnit({ description, programArguments, workingDirectory, environment }) {
	...
	const descriptionValue = description?.trim() || "OpenPaw Gateway";
...
async function installSystemdService({ env, stdout, programArguments, workingDirectory, environment, description }) {
	...
	const unitName = `${resolveGatewaySystemdServiceName(env.OPENPAW_PROFILE)}.service`;
...
async function uninstallSystemdService({ env, stdout }) {
	...
	await execSystemctl([
		...
		"disable",
		"--now",
		`${resolveGatewaySystemdServiceName(env.OPENPAW_PROFILE)}.service`
	]);
	const unitPath = resolveSystemdUnitPath(env);
	try {
...
function renderGatewayServiceStartHints(env = process.env) {
	const base = [formatCliCommand("openpaw gateway install", env), formatCliCommand("openpaw gateway", env)];
	const profile = env.OPENPAW_PROFILE;
...
async function runDaemonInstall(opts) {
	...
	if (loaded) {
		if (!opts.force) {
			...
			if (!json) {
				...
				defaultRuntime.log(`Reinstall with: ${formatCliCommand("openpaw gateway install --force")}`);
			}
			return;
		}
	}
	...
	let token = opts.token || cfg.gateway?.auth?.token || process.env.OPENPAW_GATEWAY_TOKEN || process.env.PAWBOT_GATEWAY_TOKEN;
	if (!token && needsToken) {
		token = randomToken();
...
function auditGatewayToken(command, issues, expectedGatewayToken) {
	...
	const serviceToken = command?.environment?.OPENPAW_GATEWAY_TOKEN?.trim();
	if (serviceToken === expectedToken) return;
	issues.push({
		code: SERVICE_AUDIT_CODES.gatewayTokenMismatch,
		message: "Gateway service OPENPAW_GATEWAY_TOKEN does not match gateway.auth.token in openpaw.json",
...
function checkTokenDrift(params) {
	...
	if (configToken && serviceToken !== configToken) return {
		...
		detail: "Run `openpaw gateway install --force` to sync the token.",
		level: "recommended"
	};
	return null;
}
...
function renderSystemdUnavailableHints(options = {}) {
	...
	return ["systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.", `If you're in a container, run the gateway in the foreground instead of \`${formatCliCommand("openpaw gateway")}\`.`];
}
...
async function runServiceRestart(params) {
	...
	if (params.checkTokenDrift) try {
		const serviceToken = (await params.service.readCommand(process.env))?.environment?.OPENPAW_GATEWAY_TOKEN;
...
function classifyPortListener(listener, port) {
	...
	if (raw.includes("openpaw")) return "gateway";
	if (raw.includes("ssh")) {
...
function buildPortHints(listeners, port) {
	...
	const hints = [];
	if (kinds.has("gateway")) hints.push(`Gateway already running locally. Stop it (${formatCliCommand("openpaw gateway stop")}) or use a different port.`);
...
async function runDaemonRestart(opts = {}) {
	...
	return await runServiceRestart({
		...
		postRestartCheck: async ({ warnings, fail, stdout }) => {
			...
			fail(`Gateway restart timed out after ${restartWaitSeconds}s waiting for health checks.`, [formatCliCommand("openpaw gateway status --probe --deep"), formatCliCommand("openpaw doctor")]);
		}
	});
}
...
function hasGatewayServiceMarker(content) {
	const lower = content.toLowerCase();
	const markerKeys = ["openpaw_service_marker"];
	const kindKeys = ["openpaw_service_kind"];
...
function isOpenPawGatewayLaunchdService(label, contents) {
...
function isOpenPawGatewaySystemdService(name, contents) {
	if (hasGatewayServiceMarker(contents)) return true;
	if (!name.startsWith("openpaw-gateway")) return false;
	return contents.toLowerCase().includes("gateway");
}
function isOpenPawGatewayTaskName(name) {
	const normalized = name.trim().toLowerCase();
	if (!normalized) return false;
	return normalized === resolveGatewayWindowsTaskName().toLowerCase() || normalized.startsWith("openpaw gateway");
}
...
function isLegacyLabel(label) {
	...
	return lower.includes("pawbot") || lower.includes("pawbot");
}
	...
	for (const { name: labelFromName, fullPath, contents } of candidates) {
		...
		if (!marker) {
			...
			results.push({
				...
				scope: params.scope,
				marker: isLegacyLabel(label) ? "pawbot" : "pawbot",
				legacy: true
			});
			continue;
		}
		if (isIgnoredLaunchdLabel(label)) continue;
		if (marker === "openpaw" && isOpenPawGatewayLaunchdService(label, contents)) continue;
		results.push({
			platform: "darwin",
			label,
			detail: `plist: ${fullPath}`,
...
async function scanSystemdDir(params) {
	...
	for (const { entry, name, fullPath, contents } of candidates) {
		const marker = detectMarker(contents);
		if (!marker) continue;
		if (marker === "openpaw" && isOpenPawGatewaySystemdService(name, contents)) continue;
		results.push({
			platform: "linux",
			label: entry,
...
async function findExtraGatewayServices(env, opts = {}) {
	...
	if (process.platform === "win32") {
		...
		for (const task of tasks) {
			...
			if (!name) continue;
			if (isOpenPawGatewayTaskName(name)) continue;
...
async function gatherDaemonStatus(opts) {
	...
	const rpc = opts.probe ? await probeGatewayStatus({
		url: probeUrl,
		token: opts.rpc.token || mergedDaemonEnv.OPENPAW_GATEWAY_TOKEN || daemonCfg.gateway?.auth?.token,
		password: opts.rpc.password || mergedDaemonEnv.OPENPAW_GATEWAY_PASSWORD || daemonCfg.gateway?.auth?.password,
		timeoutMs,
		json: opts.rpc.json,
...
function printDaemonStatus(status, opts) {
	...
	if (service.configAudit?.issues.length) {
		...
		defaultRuntime.error(warnText(`Recommendation: run "${formatCliCommand("openpaw doctor")}" (or "${formatCliCommand("openpaw doctor --repair")}").`));
	}
	if (status.config) {
		...
		if (status.config.mismatch) {
			...
			defaultRuntime.error(errorText(`Fix: rerun \`${formatCliCommand("openpaw gateway install --force")}\` from the same --profile / OPENPAW_STATE_DIR you expect.`));
		}
		spacer();
	}
	if (status.gateway) {
	...
	if (service.runtime?.cachedLabel) {
		const labelValue = resolveGatewayLaunchAgentLabel((service.command?.environment ?? process.env).OPENPAW_PROFILE);
		...
		defaultRuntime.error(errorText(`Then reinstall: ${formatCliCommand("openpaw gateway install")}`));
		spacer();
	}
	...
	if (service.loaded && service.runtime?.status === "running" && status.port && status.port.status !== "busy") {
		...
		if (process.platform === "linux") {
			const unit = resolveGatewaySystemdServiceName((service.command?.environment ?? process.env).OPENPAW_PROFILE);
