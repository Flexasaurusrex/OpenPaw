...
import { n as resolveOpenPawPackageRootSync, t as resolveOpenPawPackageRoot } from "./openpaw-root-BZbrf12M.js";
...
async function resolveControlUiDistIndexPath(argv1OrOpts) {
	...
	for (let i = 0; i < 8; i++) {
		...
		if (fs.existsSync(pkgJsonPath)) try {
			...
			if (JSON.parse(raw).name === "openpaw") return fs.existsSync(indexPath) ? indexPath : null;
			return null;
		} catch {
			return null;
		}
