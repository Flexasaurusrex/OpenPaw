...
import { n as resolveBrewPathDirs } from "./brew-AuKrskR-.js";
...
function candidateBinDirs(opts) {
	...
	try {
		...
		if (isExecutable(path.join(execDir, "openpaw"))) prepend.push(execDir);
	} catch {}
	if (opts.allowProjectLocalBin === true || isTruthyEnvValue(process.env.OPENPAW_ALLOW_PROJECT_LOCAL_BIN)) {
		const localBinDir = path.join(cwd, "node_modules", ".bin");
		if (isExecutable(path.join(localBinDir, "openpaw"))) append.push(localBinDir);
	}
...
*/
function ensureOpenPawCliOnPath(opts = {}) {
	if (isTruthyEnvValue(process.env.OPENPAW_PATH_BOOTSTRAPPED)) return;
...
//#endregion
export { ensureOpenPawCliOnPath as t };