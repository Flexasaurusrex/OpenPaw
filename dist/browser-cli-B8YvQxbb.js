import { g as resolveStateDir } from "./paths-5liSMgvV.js";
import { B as theme, O as danger, S as shortenHomePath, k as info } from "./utils-CLEQvfyB.js";
...
import { i as resolveExistingPathsWithinRoot, r as DEFAULT_UPLOAD_DIR } from "./paths-DX8xqj9_.js";
...
import { n as callGatewayFromCli, t as addGatewayClientOptions } from "./gateway-rpc-BikSeXeG.js";
import { t as copyToClipboard } from "./clipboard-BwWumNyL.js";
...
function registerBrowserFilesAndDownloadsCommands(browser, parentOpts) {
	...
	browser.command("upload").description("Arm file upload for the next file chooser").argument("<paths...>", "File paths to upload (must be within OpenPaw temp uploads dir, e.g. /tmp/openpaw/uploads/file.pdf)").option("--ref <ref>", "Ref id from snapshot to click after arming").option("--input-ref <ref>", "Ref id for <input type=file> to set directly").option("--element <selector>", "CSS selector for <input type=file>").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the next file chooser (default: 120000)", (v) => Number(v)).action(async (paths, opts, cmd) => {
		const { parent, profile } = resolveBrowserActionContext(cmd, parentOpts);
		try {
...
function registerBrowserDebugCommands(browser, parentOpts) {
	...
	trace.command("stop").description("Stop trace recording and write a .zip").option("--out <path>", "Output path within openpaw temp dir (e.g. trace.zip or /tmp/openpaw/trace.zip)").option("--target-id <id>", "CDP target id (or unique prefix)").action(async (opts, cmd) => {
		const parent = parentOpts(cmd);
		const profile = parent?.browserProfile;
		await runBrowserDebug(async () => {
			const result = await callBrowserRequest(parent, {
...
const browserActionExamples = [
	...
	"openpaw browser upload /tmp/openpaw/uploads/file.pdf",
...
async function installChromeExtension(opts) {
	...
	if (!hasManifest(src)) throw new Error("Bundled Chrome extension is missing. Reinstall OpenPaw and try again.");
...
function registerBrowserExtensionCommands(browser, parentOpts) {
	...
	ext.command("path").description("Print the path to the installed Chrome extension (load unpacked)").action(async (_opts, cmd) => {
		...
		if (!hasManifest(dir)) {
			defaultRuntime.error(danger([`Chrome extension is not installed. Run: "${formatCliCommand("openpaw browser extension install")}"`, `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.openpaw.ai/tools/chrome-extension")}`].join("\n")));
			defaultRuntime.exit(1);
		}
		if (parent?.json) {
...
function registerBrowserManageCommands(browser, parentOpts) {
	...
	browser.command("create-profile").description("Create a new browser profile").requiredOption("--name <name>", "Profile name (lowercase, numbers, hyphens)").option("--color <hex>", "Profile color (hex format, e.g. #0066CC)").option("--cdp-url <url>", "CDP URL for remote Chrome (http/https)").option("--driver <driver>", "Profile driver (openpaw|extension). Default: openpaw").action(async (opts, cmd) => {
		const parent = parentOpts(cmd);
		await runBrowserCommand$1(async () => {
			const result = await callBrowserRequest(parent, {
				method: "POST",
...
function registerBrowserCli(program) {
	const browser = program.command("browser").description("Manage OpenPaw's dedicated browser (Chrome/Chromium)").option("--browser-profile <name>", "Browser profile name (default from config)").option("--json", "Output machine-readable JSON", false).addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([...browserCoreExamples, ...browserActionExamples].map((cmd) => [cmd, ""]), true)}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/browser", "docs.openpaw.ai/cli/browser")}\n`).action(() => {
		browser.outputHelp();
		defaultRuntime.error(danger(`Missing subcommand. Try: "${formatCliCommand("openpaw browser status")}"`));
		defaultRuntime.exit(1);
	});
