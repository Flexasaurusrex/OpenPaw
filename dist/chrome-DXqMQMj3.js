import { t as CONFIG_DIR } from "./utils-CLEQvfyB.js";
import { t as createSubsystemLogger } from "./subsystem-BlU6Lg5x.js";
import { i as loadConfig } from "./config-D2VtfSUL.js";
import { i as isLoopbackHost, r as isLoopbackAddress, t as rawDataToString } from "./ws-DI10x-Zu.js";
import { a as resolvePinnedHostnameWithPolicy } from "./ssrf-CVJYDLbF.js";
import { r as ensurePortAvailable } from "./ports-7zKdGljn.js";
...
const DEFAULT_OPENPAW_BROWSER_ENABLED = true;
const DEFAULT_BROWSER_EVALUATE_ENABLED = true;
const DEFAULT_OPENPAW_BROWSER_COLOR = "#FF4500";
const DEFAULT_OPENPAW_BROWSER_PROFILE_NAME = "openpaw";
...
const RELAY_TOKEN_CONTEXT = "openpaw-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const OPENPAW_RELAY_BROWSER = "OpenPaw/extension-relay";
function resolveGatewayAuthToken() {
	const envToken = process.env.OPENPAW_GATEWAY_TOKEN?.trim() || process.env.PAWBOT_GATEWAY_TOKEN?.trim();
...
function resolveRelayAuthTokenForPort(port) {
	...
	throw new Error("extension relay requires gateway auth token (set gateway.auth.token or OPENPAW_GATEWAY_TOKEN)");
}
async function probeAuthenticatedOpenPawRelay(params) {
	...
	try {
		...
		return (typeof body?.Browser === "string" ? body.Browser.trim() : "") === OPENPAW_RELAY_BROWSER;
	} catch {
		return false;
	} finally {
		clearTimeout(timer);
...
const RELAY_AUTH_HEADER = "x-openpaw-relay-token";
...
async function ensureChromeExtensionRelayServer(opts) {
	...
	try {
		...
		if (isAddrInUseError(err) && await probeAuthenticatedOpenPawRelay({
...
*/
function decorateOpenPawProfile(userDataDir, opts) {
...
function resolveOpenPawUserDataDir(profileName = DEFAULT_OPENPAW_BROWSER_PROFILE_NAME) {
...
async function launchOpenPawChrome(resolved, profile) {
	...
	const needsDecorate = !isProfileDecorated(userDataDir, profile.name, (profile.color ?? DEFAULT_OPENPAW_BROWSER_COLOR).toUpperCase());
	const spawnOnce = () => {
		const args = [
	...
	log.info(`🐾 openpaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`);
	return {
		pid,
		exe,
		userDataDir,
...
async function stopOpenPawChrome(running, timeoutMs = 2500) {
...
//#endregion
export { DEFAULT_OPENPAW_BROWSER_PROFILE_NAME as A, DEFAULT_AI_SNAPSHOT_EFFICIENT_DEPTH as C, DEFAULT_BROWSER_EVALUATE_ENABLED as D, DEFAULT_BROWSER_DEFAULT_PROFILE_NAME as E, DEFAULT_OPENPAW_BROWSER_COLOR as O, stopChromeExtensionRelayServer as S, DEFAULT_AI_SNAPSHOT_MAX_CHARS as T, fetchJson as _, resolveOpenPawUserDataDir as a, withCdpSocket as b, captureScreenshot as c, normalizeCdpWsUrl as d, snapshotAria as f, appendCdpPath as g, withBrowserNavigationPolicy as h, launchOpenPawChrome as i, DEFAULT_OPENPAW_BROWSER_ENABLED as k, createTargetViaCdp as l, assertBrowserNavigationAllowed as m, isChromeCdpReady as n, stopOpenPawChrome as o, InvalidBrowserNavigationUrlError as p, isChromeReachable as r, resolveBrowserExecutableForPlatform as s, getChromeWebSocketUrl as t, formatAriaSnapshot as u, fetchOk as v, DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS as w, ensureChromeExtensionRelayServer as x, getHeadersWithAuth as y };