import { n as DEFAULT_GATEWAY_PORT, u as resolveGatewayPort } from "./paths-5liSMgvV.js";
...
async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-C5liWlpx.js").then((n) => n.d);
	...
	if (snapshot.exists && !snapshot.valid) {
		...
		await prompter.outro(`Config invalid. Run \`${formatCliCommand("openpaw doctor")}\` to repair it, then re-run onboarding.`);
		runtime.exit(1);
		return;
	}
	...
	if (flow === "quickstart" && !explicitFlow) await prompter.note("Smart move! Let's get you up and running fast 🐾", "QuickStart");
	else if (flow === "advanced" && !explicitFlow) await prompter.note("Ah, a power user. I respect that. Let's dive deep 🔧", "Advanced");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-D9jFUepF.js").then((n) => n.t);
		const { logConfigUpdated } = await import("./logging-CZH6rk0L.js").then((n) => n.r);
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-Czeu1Jtv.js").then((n) => n.t);
	const { promptCustomApiConfig } = await import("./onboard-custom-DGLlVcCk.js").then((n) => n.r);
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-CAhCq6sk.js").then((n) => n.t);
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-Dpm45Jn1.js").then((n) => n.i);
	...
	if (authChoice !== "skip") await prompter.note("Your secrets are safe with me. Unlike your .env file that one time... 🔐", "Auth configured");
	...
	const { configureGatewayForOnboarding } = await import("./onboarding.gateway-config-DbmECCEZ.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-PixIvqwD.js").then((n) => n.i);
		const { setupChannels } = await import("./onboard-channels-D9ss7hvZ.js").then((n) => n.n);
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-D95z_RGt.js").then((n) => n.t);
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-BdWpguUn.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeOnboardingWizard } = await import("./onboarding.finalize-B4VoIiEi.js");
