...
var OpenPawA2UIHost = class extends i$6 {
	static properties = {
		surfaces: { state: true },
  ...
  `;
	...
	#handleA2UIAction(evt) {
		...
		globalThis.__openpawLastA2UIAction = userAction;
		const handler = globalThis.webkit?.messageHandlers?.openpawCanvasA2UIAction ?? globalThis.openpawCanvasA2UIAction;
		if (handler?.postMessage) {
			try {
				if (handler === globalThis.openpawCanvasA2UIAction) {
...
if (!customElements.get("openpaw-a2ui-host")) {
	customElements.define("openpaw-a2ui-host", OpenPawA2UIHost);
}
