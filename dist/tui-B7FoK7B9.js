...
import { l as stripAnsi$1, u as visibleWidth } from "./subsystem-BlU6Lg5x.js";
import { C as parseAgentSessionKey, c as normalizeMainKey, r as buildAgentMainSessionKey, s as normalizeAgentId } from "./session-key-Dx_ARweb.js";
import { f as GATEWAY_CLIENT_CAPS, h as GATEWAY_CLIENT_NAMES, m as GATEWAY_CLIENT_MODES } from "./message-channel-DlEv3rvX.js";
import { i as loadConfig } from "./config-D2VtfSUL.js";
import { t as VERSION } from "./version-CmTNPgEd.js";
import { _ as formatRawAssistantErrorForUi } from "./pi-embedded-helpers-DHAZhpow.js";
import { i as stripLeadingInboundMetadata } from "./chat-envelope-Ddj7Occv.js";
import { n as resolveToolDisplay, t as formatToolDetail } from "./tool-display-VQ2UFB5v.js";
import { a as listChatCommandsForConfig, i as listChatCommands } from "./commands-registry-Dz5ewNx0.js";
import { At as PROTOCOL_VERSION, t as GatewayClient } from "./client-8n3vPrA3.js";
import { i as ensureExplicitGatewayAuth, o as resolveExplicitGatewayAuth, t as buildGatewayConnectionDetails } from "./call-qMGOPQLT.js";
import { n as formatTimeAgo, t as formatRelativeTimestamp } from "./format-relative-CnogjKdn.js";
import { n as formatTokenCount } from "./usage-format-DP3TmK6B.js";
...
function resolveGatewayConnection(opts) {
	...
	return {
		...
		token: explicitAuth.token || (!urlOverride ? isRemoteMode ? typeof remote?.token === "string" && remote.token.trim().length > 0 ? remote.token.trim() : void 0 : process.env.OPENPAW_GATEWAY_TOKEN?.trim() || (typeof authToken === "string" && authToken.trim().length > 0 ? authToken.trim() : void 0) : void 0),
		password: explicitAuth.password || (!urlOverride ? process.env.OPENPAW_GATEWAY_PASSWORD?.trim() || (typeof remote?.password === "string" && remote.password.trim().length > 0 ? remote.password.trim() : void 0) : void 0)
	};
}
...
function resolveGatewayDisconnectState(reason) {
	...
	if (/pairing required/i.test(reasonLabel)) return {
		...
		activityStatus: "pairing required: run openpaw devices list",
		pairingHint: "Pairing required. Run `openpaw devices list`, approve your request ID, then reconnect."
	};
	return {
		connectionStatus: `gateway disconnected: ${reasonLabel}`,
		activityStatus: "idle"
...
async function runTui(opts) {
	...
	const updateHeader = () => {
		...
		header.setText(theme.header(`openpaw tui - ${client.connection.url} - agent ${agentLabel} - session ${sessionLabel}`));
	};
