...
function formatSkillsList(report, opts) {
	...
	if (skills.length === 0) return appendClawHubHint(opts.eligible ? `No eligible skills found. Run \`${formatCliCommand("openpaw skills list")}\` to see all skills.` : "No skills found.", opts.json);
...
function formatSkillInfo(report, skillName, opts) {
	...
	if (!skill) {
		...
		return appendClawHubHint(`Skill "${skillName}" not found. Run \`${formatCliCommand("openpaw skills list")}\` to see available skills.`, opts.json);
	}
...
async function loadSkillsStatusReport() {
	...
	const { buildWorkspaceSkillStatus } = await import("./skills-status-ITwt8ZlC.js").then((n) => n.n);
