import { d as resolveIsNixMode, g as resolveStateDir, m as resolveOAuthDir, t as CONFIG_PATH, u as resolveGatewayPort, y as resolveRequiredHomeDir } from "./paths-5liSMgvV.js";
import { C as sleep, S as shortenHomePath, U as getResolvedLoggerSettings, y as resolveUserPath } from "./utils-CLEQvfyB.js";
import { _r as DEFAULT_OAUTH_WARN_MS, vr as buildAuthHealthSummary, vt as loadOpenPawPlugins, yr as formatRemainingShort } from "./reply-D_97LfkS.js";
import { i as resolveAgentDir, l as resolveAgentWorkspaceDir, m as DEFAULT_AGENTS_FILENAME, u as resolveDefaultAgentId } from "./agent-scope-DVw9WyO3.js";
import { f as defaultRuntime } from "./subsystem-BlU6Lg5x.js";
import { t as resolveOpenPawPackageRoot } from "./openpaw-root-BZbrf12M.js";
import { n as runExec, t as runCommandWithTimeout } from "./exec-DkdDR1S0.js";
import { Gt as CODEX_CLI_PROFILE_ID, J as resolveApiKeyForProvider, Jt as DEFAULT_PROVIDER, Mt as ensureAuthProfileStore, Pt as updateAuthProfileStoreWithLock, Tt as repairOAuthProfileIdMismatch, Wt as CLAUDE_CLI_PROFILE_ID, a as getModelRefStatus, bt as resolveApiKeyForProfile, h as resolveHooksGmailModel, p as resolveConfiguredModelRef, qt as DEFAULT_MODEL, vt as resolveProfileUnusableUntilForDisplay } from "./model-selection-DLFgSmxJ.js";
import { t as formatCliCommand } from "./command-format-BMHSLXMa.js";
import { t as isTruthyEnvValue } from "./env-BuaPy6k5.js";
import { l as writeConfigFile, o as readConfigFileSnapshot } from "./config-D2VtfSUL.js";
import { A as DEFAULT_SANDBOX_COMMON_IMAGE, j as DEFAULT_SANDBOX_IMAGE, k as DEFAULT_SANDBOX_BROWSER_IMAGE, p as resolveSandboxScope } from "./sandbox-DfsyhnGI.js";
import { d as resolveGatewayBindHost, i as isLoopbackHost } from "./ws-DI10x-Zu.js";
import { a as resolveGatewayAuth } from "./auth-DiCh-IhS.js";
import { a as inspectPortUsage, c as formatPortDiagnostics } from "./ports-7zKdGljn.js";
import { $ as cleanStaleLockFiles, K as resolveMainSessionKey, s as loadSessionStore } from "./sessions-DrdLhqGQ.js";
import { n as listChannelPlugins } from "./plugins-PixIvqwD.js";
import { c as resolveSessionTranscriptsDirForAgent, i as resolveSessionFilePathOptions, l as resolveStorePath, r as resolveSessionFilePath } from "./paths-UwFQANoB.js";
import { n as loadModelCatalog } from "./model-catalog-WZgV-OzI.js";
import { i as resolveMemoryBackendConfig } from "./memory-cli-BB0oE2Bs.js";
import { n as resolveMemorySearchConfig } from "./manager-DoMFtXnm.js";
import { n as callGateway, t as buildGatewayConnectionDetails } from "./call-qMGOPQLT.js";
import { r as isWSLEnv, t as isWSL } from "./wsl-BCmcMXD3.js";
import { h as randomToken, n as applyWizardMetadata, p as printWizardHeader, s as guardCancel } from "./onboard-helpers-C5liWlpx.js";
import { n as stylePromptMessage, r as stylePromptTitle, t as stylePromptHint } from "./prompt-style-DE_Wto62.js";
import { f as resolveGatewaySystemdServiceName, h as resolveNodeLaunchAgentLabel, l as resolveGatewayLaunchAgentLabel, p as resolveGatewayWindowsTaskName } from "./constants-mKw6G1y5.js";
import { t as buildWorkspaceSkillStatus } from "./skills-status-Cuhn-Hl6.js";
import { t as note$1 } from "./note-UN8_Sj2N.js";
import { t as collectChannelStatusIssues } from "./channels-status-issues-CwTGWQnj.js";
import { d as resolveSystemNodeInfo, l as renderSystemNodeWarning, n as gatewayInstallErrorHint, t as buildGatewayInstallPlan } from "./daemon-install-helpers-CUrSwNMr.js";
import { n as GATEWAY_DAEMON_RUNTIME_OPTIONS, t as DEFAULT_GATEWAY_DAEMON_RUNTIME } from "./daemon-runtime-DG_wSD1u.js";
import { l as uninstallLegacySystemdUnits, r as isSystemdUserServiceAvailable } from "./systemd-DwAFBjTQ.js";
import { a as launchAgentPlistExists, i as isLaunchAgentLoaded, o as repairLaunchAgentBootstrap, r as isLaunchAgentListed, s as resolveGatewayLogPaths, t as resolveGatewayService } from "./service-C4VX1Mu3.js";
import { i as auditGatewayServiceConfig, n as renderSystemdUnavailableHints, o as needsNodeRuntimeMigration, r as SERVICE_AUDIT_CODES, s as formatRuntimeStatus, t as isSystemdUnavailableDetail } from "./systemd-hints-k_ESxlcH.js";
import { t as readLastGatewayErrorLine } from "./diagnostics-CyFx5d3I.js";
import { n as renderGatewayServiceCleanupHints, t as findExtraGatewayServices } from "./inspect-D2e9qagv.js";
import { r as healthCommand } from "./health-CGlzI1Dh.js";
import { n as resolveControlUiDistIndexHealth, r as resolveControlUiDistIndexPathForRoot } from "./control-ui-assets-CNr0fISG.js";
import { t as resolveDefaultChannelAccountContext } from "./channel-account-context-COpC9CMr.js";
import { t as resolveDmAllowState } from "./dm-policy-shared-BWPWS3uZ.js";
import { n as runGatewayUpdate, t as resolveAgentSessionDirs } from "./session-dirs-BBPI9K3m.js";
import { n as logConfigUpdated } from "./logging-CZH6rk0L.js";
import { n as detectLegacyStateMigrations, r as runLegacyStateMigrations, t as loadAndMaybeMigrateDoctorConfig } from "./doctor-config-flow-CQ5pMyXF.js";
import { t as ensureSystemdUserLingerInteractive } from "./systemd-linger-vJVhrluo.js";
import { t as formatHealthCheckFailure } from "./health-format-FKtSoCB_.js";
import { n as doctorShellCompletion } from "./doctor-completion-CJ6O2sEB.js";
...
async function maybeRemoveDeprecatedCliAuthProfiles(cfg, prompter) {
	...
	if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) lines.push(`- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("openpaw models auth setup-token")}`);
	if (deprecated.has(CODEX_CLI_PROFILE_ID)) lines.push(`- ${CODEX_CLI_PROFILE_ID} (OpenAI Codex): use OAuth → ${formatCliCommand("openpaw models auth login --provider openai-codex")}`);
	note$1(lines.join("\n"), "Auth profiles");
...
function formatAuthIssueHint(issue) {
	if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) return `Deprecated profile. Use ${formatCliCommand("openpaw models auth setup-token")} or ${formatCliCommand("openpaw configure")}.`;
	if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) return `Deprecated profile. Use ${formatCliCommand("openpaw models auth login --provider openai-codex")} or ${formatCliCommand("openpaw configure")}.`;
...
async function maybeRepairGatewayDaemon(params) {
	...
	if (process.platform === "darwin") {
		...
		note$1(`LaunchAgent loaded; stopping requires "${formatCliCommand("openpaw gateway stop")}" or launchctl bootout gui/$UID/${label}.`, "Gateway");
	}
	if (serviceRuntime?.status === "running") {
		if (await params.prompter.confirmSkipInNonInteractive({
...
function resolveGatewayAuthToken(cfg, env) {
	...
	return (env.OPENPAW_GATEWAY_TOKEN ?? env.PAWBOT_GATEWAY_TOKEN)?.trim() || void 0;
}
...
async function maybeScanExtraGatewayServices(options, runtime, prompter) {
	...
	if (legacyServices.length > 0) {
		if (await prompter.confirmSkipInNonInteractive({
			message: "Remove legacy gateway services (pawbot/pawbot) now?",
			initialValue: true
		})) {
			const removed = [];
			...
			if (removed.length > 0) runtime.log("Legacy gateway services removed. Installing OpenPaw gateway next.");
		}
	}
...
* Runs as part of `openpaw doctor` — config-only, no network calls.
*/
async function noteMemorySearchHealth(cfg) {
	...
	if (resolved.provider !== "auto") {
		if (resolved.provider === "local") {
			...
			note$1([
				...
				`- Switch to a remote provider: ${formatCliCommand("openpaw config set agents.defaults.memorySearch.provider openai")}`,
				"",
				`Verify: ${formatCliCommand("openpaw memory status --deep")}`
			].join("\n"), "Memory search");
			return;
		}
		...
		note$1([
			...
			`- Add credentials: ${formatCliCommand(`openpaw auth add --provider ${resolved.provider}`)}`,
			`- To disable: ${formatCliCommand("openpaw config set agents.defaults.memorySearch.enabled false")}`,
			"",
			`Verify: ${formatCliCommand("openpaw memory status --deep")}`
		].join("\n"), "Memory search");
		return;
	...
	note$1([
		...
		`- Add credentials: ${formatCliCommand("openpaw auth add --provider openai")}`,
...
async function noteMacLaunchAgentOverrides() {
	...
	const markerPath = [path.join(home, ".openpaw", "disable-launchagent")].find((candidate) => fs.existsSync(candidate));
...
async function noteMacLaunchctlGatewayEnvOverrides(cfg, deps) {
	...
	const deprecatedLaunchctlEntries = [["PAWBOT_GATEWAY_TOKEN", await getenv("PAWBOT_GATEWAY_TOKEN")], ["PAWBOT_GATEWAY_PASSWORD", await getenv("PAWBOT_GATEWAY_PASSWORD")]].filter((entry) => Boolean(entry[1]?.trim()));
	if (deprecatedLaunchctlEntries.length > 0) {
		const lines = ["- Deprecated launchctl environment variables detected (ignored).", ...deprecatedLaunchctlEntries.map(([key]) => `- \`${key}\` is set; use \`OPENPAW_${key.slice(key.indexOf("_") + 1)}\` instead.`)];
		(deps?.noteFn ?? note$1)(lines.join("\n"), "Gateway (macOS)");
	}
	const tokenEntries = [["OPENPAW_GATEWAY_TOKEN", await getenv("OPENPAW_GATEWAY_TOKEN")]];
	const passwordEntries = [["OPENPAW_GATEWAY_PASSWORD", await getenv("OPENPAW_GATEWAY_PASSWORD")]];
	...
	const lines = [
		...
		envPassword ? `- \`${envPasswordKey ?? "OPENPAW_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.` : void 0,
...
function noteDeprecatedLegacyEnvVars(env = process.env, deps) {
	const entries = Object.entries(env).filter(([key, value]) => key.startsWith("PAWBOT_") && value?.trim()).map(([key]) => key);
	if (entries.length === 0) return;
	const lines = [
		"- Deprecated legacy environment variables detected (ignored).",
		"- Use OPENPAW_* equivalents instead:",
		...entries.map((key) => {
			return `  ${key} -> OPENPAW_${key.slice(key.indexOf("_") + 1)}`;
		})
	];
	(deps?.noteFn ?? note$1)(lines.join("\n"), "Environment");
}
...
async function noteSecurityWarnings(cfg) {
	...
	if (cfg.approvals?.exec?.enabled === false) warnings.push("- Note: approvals.exec.enabled=false disables approval forwarding only.", "  Host exec gating still comes from ~/.openpaw/exec-approvals.json.", `  Check local policy with: ${formatCliCommand("openpaw approvals get --gateway")}`);
	...
	if (isExposed) if (!hasSharedSecret) {
		const authFixLines = resolvedAuth.mode === "password" ? [`  Fix: ${formatCliCommand("openpaw configure")} to set a password`, `  Or switch to token: ${formatCliCommand("openpaw config set gateway.auth.mode token")}`] : [`  Fix: ${formatCliCommand("openpaw doctor --fix")} to generate a token`, `  Or set token directly: ${formatCliCommand("openpaw config set gateway.auth.mode token")}`];
		warnings.push(`- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`, `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`, `  Fix: ${formatCliCommand("openpaw config set gateway.bind loopback")}`, ...saferRemoteAccessLines, ...authFixLines);
	...
	const warnDmPolicy = async (params) => {
		...
		if (dmScope === "main" && isMultiUserDm) warnings.push(`- ${params.label} DMs: multiple senders share the main session; run: ` + formatCliCommand("openpaw config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate sessions.");
	};
...
async function noteSessionLockHealth(params) {
	...
	if (staleCount > 0 && !shouldRepair) {
		...
		lines.push("- Run \"openpaw doctor --fix\" to remove stale lock files automatically.");
	}
...
function shouldRequireOAuthDir(cfg, env) {
	if (env.OPENPAW_OAUTH_DIR?.trim()) return true;
...
async function detectOpenPawGitCheckout(root) {
...
async function maybeOfferUpdateBeforeDoctor(params) {
	if (!(!isTruthyEnvValue(process.env.OPENPAW_UPDATE_IN_PROGRESS) && params.options.nonInteractive !== true && params.options.yes !== true && params.options.repair !== true && Boolean(process.stdin.isTTY)) || !params.root) return { updated: false };
	...
	if (git === "not-git") note$1(["This install is not a git checkout.", `Run \`${formatCliCommand("openpaw update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`].join("\n"), "Update");
	return { updated: false };
}
...
async function doctorCommand(runtime = defaultRuntime, options = {}) {
	...
	if (!cfg.gateway?.mode) {
		...
		if (!fs.existsSync(configPath)) lines.push(`Missing config: run ${formatCliCommand("openpaw setup")} first.`);
		note$1(lines.join("\n"), "Gateway");
	}
	...
	} else runtime.log(`Run "${formatCliCommand("openpaw doctor --fix")}" to apply changes.`);
