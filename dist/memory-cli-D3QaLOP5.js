...
import { B as theme, P as setVerbose, R as colorize, S as shortenHomePath, x as shortenHomeInString, y as resolveUserPath, z as isRich } from "./utils-CLEQvfyB.js";
...
import { C as parseDurationMs, i as loadConfig, ut as splitShellArgs } from "./config-BrzzL48f.js";
...
import { f as listMemoryFiles, p as normalizeExtraMemoryPaths } from "./query-expansion-C0LYSFr9.js";
...
import { n as withProgress, r as withProgressTotals } from "./progress-BrNyn1gy.js";
...
async function getMemorySearchManager(params) {
	...
	if (resolved.backend === "qmd" && resolved.qmd) {
		...
		try {
			const { QmdMemoryManager } = await import("./qmd-manager-BC-dfwS2.js");
			...
			if (primary) {
				...
				const wrapper = new FallbackMemoryManager({
					...
					fallbackFactory: async () => {
						const { MemoryIndexManager } = await import("./manager-J8EoHgoj.js").then((n) => n.t);
...
function registerMemoryCli(program) {
	const memory = program.command("memory").description("Search, inspect, and reindex memory files").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["openpaw memory status", "Show index and provider status."],
		["openpaw memory index --force", "Force a full reindex."],
		["openpaw memory search --query \"deployment notes\"", "Search indexed memory entries."],
		["openpaw memory status --json", "Output machine-readable JSON."]
