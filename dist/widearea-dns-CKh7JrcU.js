import { c as ensureDir, t as CONFIG_DIR } from "./utils-CLEQvfyB.js";
...
function resolveWideAreaDiscoveryDomain(params) {
	...
	return normalizeWideAreaDomain(params?.configDomain ?? env.OPENPAW_WIDE_AREA_DOMAIN ?? null);
}
...
function extractContentHash(zoneText) {
	return zoneText.match(/^\s*;\s*openpaw-content-hash:\s*(\S+)\s*$/m)?.[1] ?? null;
}
...
function renderZone(opts) {
	...
	records.push(`_openpaw-gw._tcp IN PTR ${instanceLabel}._openpaw-gw._tcp`);
	records.push(`${instanceLabel}._openpaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
	records.push(`${instanceLabel}._openpaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
	const contentBody = `${records.join("\n")}\n`;
	return `; openpaw-content-hash: ${computeContentHash(`${records.map((line) => line === soaLine ? `@ IN SOA ns1 hostmaster SERIAL 7200 3600 1209600 60` : line).join("\n")}\n`)}\n${contentBody}`;
}
