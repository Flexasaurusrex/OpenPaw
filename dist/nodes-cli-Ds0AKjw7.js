...
import { B as theme, S as shortenHomePath, x as shortenHomeInString, z as isRich } from "./utils-CLEQvfyB.js";
import { r as resolveAgentConfig, u as resolveDefaultAgentId } from "./agent-scope-DVw9WyO3.js";
...
import { a as randomIdempotencyKey } from "./call-qMGOPQLT.js";
...
import { f as resolveExecApprovalsFromFile, i as maxAsk, o as minSecurity, t as DEFAULT_EXEC_APPROVAL_TIMEOUT_MS } from "./exec-approvals-FygPo23-.js";
import { a as canvasSnapshotTempPath, c as parseCameraClipPayload, d as writeCameraClipPayloadToFile, f as writeUrlToFile, i as parseEnvPairs, l as parseCameraSnapPayload, m as applyPathPrepend, n as screenRecordTempPath, o as parseCanvasSnapshotPayload, p as buildNodeShellCommand, r as writeScreenRecordToFile, s as cameraTempPath, t as parseScreenRecordPayload, u as writeBase64ToFile } from "./nodes-screen-BsqoKcbB.js";
import { n as parseNodeList, r as parsePairingList } from "./node-match-DsMVdSpH.js";
import { t as parseTimeoutMs } from "./parse-timeout-DDSWreYs.js";
import { t as renderTable } from "./table-BgQdk22X.js";
import { a as unauthorizedHintForMessage, i as resolveNodeId, n as callGatewayCli, o as formatPermissions, r as nodesCallOpts, t as buildNodeInvokeParams } from "./rpc-DluYLbTl.js";
...
function registerNodesCanvasCommands(nodes) {
	...
	nodesCallOpts(a2ui.command("push").description("Push A2UI JSONL to the canvas").option("--jsonl <path>", "Path to JSONL payload").option("--text <text>", "Render a quick A2UI text payload").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").option("--invoke-timeout <ms>", "Node invoke timeout in ms").action(async (opts) => {
		await runNodesCommand("canvas a2ui push", async () => {
			...
			if (version === "v0.9") throw new Error("Detected A2UI v0.9 JSONL (createSurface). OpenPaw currently supports v0.8 only.");
...
function registerNodesPushCommand(nodes) {
	nodesCallOpts(nodes.command("push").description("Send an APNs test push to an iOS node").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").option("--title <text>", "Push title", "OpenPaw").option("--body <text>", "Push body").option("--environment <sandbox|production>", "Override APNs environment").action(async (opts) => {
		await runNodesCommand("push", async () => {
			const nodeId = await resolveNodeId(opts, String(opts.node ?? ""));
			const title = String(opts.title ?? "").trim() || "OpenPaw";
...
function registerNodesCli(program) {
	const nodes = program.command("nodes").description("Manage gateway-owned nodes (pairing, status, invoke, and media)").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["openpaw nodes status", "List known nodes with live status."],
		["openpaw nodes pairing pending", "Show pending node pairing requests."],
		["openpaw nodes run --node <id> --raw \"uname -a\"", "Run a shell command on a node."],
		["openpaw nodes camera snap --node <id>", "Capture a photo from a node camera."]
