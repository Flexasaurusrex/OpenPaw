...
import { l as writeConfigFile, o as readConfigFileSnapshot, xt as isBlockedObjectKey } from "./config-D2VtfSUL.js";
...
import { t as redactConfigObject } from "./redact-snapshot-CUPx_McV.js";
...
async function loadValidConfig(runtime = defaultRuntime) {
	...
	runtime.error(`Run \`${formatCliCommand("openpaw doctor")}\` to repair, then retry.`);
	runtime.exit(1);
	return snapshot;
}
...
function registerConfigCli(program) {
	const cmd = program.command("config").description("Non-interactive config helpers (get/set/unset). Run without subcommand for the setup wizard.").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openpaw.ai/cli/config")}\n`).option("--section <section>", "Configure wizard sections (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
		const { configureCommandFromSectionsArg } = await import("./configure-Bb9lOilr.js").then((n) => n.t);
