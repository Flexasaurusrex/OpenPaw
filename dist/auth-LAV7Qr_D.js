...
import { a as isTrustedProxyAddress, l as resolveClientIp, n as isLoopbackAddress, t as isLocalishHost } from "./net-BVkY67bp.js";
import { n as resolveGatewayCredentialsFromValues } from "./credentials-BuewhDA6.js";
import { c as readTailscaleWhoisIdentity } from "./tailscale-QhdHV80-.js";
...
function assertGatewayAuthConfigured(auth) {
	if (auth.mode === "none") {
		console.warn("⚠️  SECURITY WARNING: Gateway running with auth mode 'none' - all connections allowed without authentication!");
		console.warn("   For production use, set gateway.auth.mode to 'token' or 'password' in openpaw.json");
	}
	if (auth.mode === "token" && !auth.token) {
		if (auth.allowTailscale) return;
		throw new Error("gateway auth mode is token, but no token was configured (set gateway.auth.token or OPENPAW_GATEWAY_TOKEN)");
	}
