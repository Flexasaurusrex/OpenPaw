...
import { A as isVerbose } from "./utils-CLEQvfyB.js";
import { Fr as estimateBase64DecodedBytes, h as sniffMimeFromBase64 } from "./reply-C7c1Otq4.js";
import { s as shouldLogSubsystemToConsole, t as createSubsystemLogger } from "./subsystem-BlU6Lg5x.js";
import { C as parseAgentSessionKey } from "./session-key-Dx_ARweb.js";
import { n as redactSensitiveText, t as getDefaultRedactPatterns } from "./redact-iq6ANRhF.js";
...
async function resolveApnsAuthConfigFromEnv(env = process.env) {
	const teamId = normalizeNonEmptyString(env.OPENPAW_APNS_TEAM_ID);
	const keyId = normalizeNonEmptyString(env.OPENPAW_APNS_KEY_ID);
	if (!teamId || !keyId) return {
		ok: false,
		error: "APNs auth missing: set OPENPAW_APNS_TEAM_ID and OPENPAW_APNS_KEY_ID"
	};
	const inlineKeyRaw = normalizeNonEmptyString(env.OPENPAW_APNS_PRIVATE_KEY_P8) ?? normalizeNonEmptyString(env.OPENPAW_APNS_PRIVATE_KEY);
	...
	const keyPath = normalizeNonEmptyString(env.OPENPAW_APNS_PRIVATE_KEY_PATH);
	if (!keyPath) return {
		ok: false,
		error: "APNs private key missing: set OPENPAW_APNS_PRIVATE_KEY_P8 or OPENPAW_APNS_PRIVATE_KEY_PATH"
	};
	try {
		return {
			ok: true,
		...
		return {
			ok: false,
			error: `failed reading OPENPAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${err instanceof Error ? err.message : String(err)}`
		};
	}
}
...
function createOpenPawPushMetadata(params) {
	return {
		kind: params.kind,
...
async function sendApnsAlert(params) {
	const payload = {
		...
		openpaw: createOpenPawPushMetadata({
			kind: "push.test",
			nodeId: params.nodeId
		})
	};
