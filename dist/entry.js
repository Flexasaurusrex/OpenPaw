...
function resolveProfileStateDir(profile, env, homedir) {
	...
	return path.join(resolveRequiredHomeDir(env, homedir), `.openpaw${suffix}`);
}
function applyCliProfileEnv(params) {
	...
	const stateDir = env.OPENPAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
	if (!env.OPENPAW_STATE_DIR?.trim()) env.OPENPAW_STATE_DIR = stateDir;
	if (!env.OPENPAW_CONFIG_PATH?.trim()) env.OPENPAW_CONFIG_PATH = path.join(stateDir, "openpaw.json");
	if (profile === "dev" && !env.OPENPAW_GATEWAY_PORT?.trim()) env.OPENPAW_GATEWAY_PORT = "19001";
}
...
function buildParseArgv(params) {
	...
	const normalizedArgv = programName && baseArgv[0] === programName ? baseArgv.slice(1) : baseArgv[0]?.endsWith("openpaw") ? baseArgv.slice(1) : baseArgv;
...
const CHAT_CHANNEL_META = {
	telegram: {
		...
		selectionExtras: ["https://openpaw.ai"]
	},
	whatsapp: {
		id: "whatsapp",
		label: "WhatsApp",
...
if (!isMainModule({
	...
	wrapperEntryPairs: [...[{
		wrapperBasename: "openpaw.mjs",
		entryBasename: "entry.js"
	}, {
		wrapperBasename: "openpaw.js",
		entryBasename: "entry.js"
	}]]
})) {} else {
	process$1.title = "openpaw";
	...
	function ensureExperimentalWarningSuppressed() {
		...
		if (isTruthyEnvValue(process$1.env.OPENPAW_NO_RESPAWN)) return false;
		if (isTruthyEnvValue(process$1.env.OPENPAW_NODE_OPTIONS_READY)) return false;
		if (hasExperimentalWarningSuppressed()) return false;
		process$1.env.OPENPAW_NODE_OPTIONS_READY = "1";
		...
		child.once("error", (error) => {
			console.error("[openpaw] Failed to respawn CLI:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exit(1);
		});
		return true;
	}
