...
import { r as resolveCliName, t as formatCliCommand } from "./command-format-Dzi36QGl.js";
import { g as DEFAULT_BOOTSTRAP_FILENAME } from "./agent-scope-Ch2id45o.js";
...
import { b as waitForGatewayReachable, f as openUrl, g as resolveControlUiLinks, i as detectBrowserOpenSupport, m as probeGatewayReachable, o as formatControlUiSshHint } from "./onboard-helpers-CQbFd_nK.js";
...
async function finalizeOnboardingWizard(options) {
	...
	if (process.platform === "linux" && systemdAvailable) {
		const { ensureSystemdUserLingerInteractive } = await import("./systemd-linger-BN4-wTtI.js").then((n) => n.r);
	...
	if (!opts.skipUi && gatewayProbe.ok) {
		...
		await prompter.note([
			...
			`View token: ${formatCliCommand("openpaw config get gateway.auth.token")}`,
			`Generate token: ${formatCliCommand("openpaw doctor --generate-gateway-token")}`,
			"Web UI stores a copy in this browser's localStorage (openpaw.control.settings.v1).",
			`Open the dashboard anytime: ${formatCliCommand("openpaw dashboard --no-open")}`,
		...
		if (hatchChoice === "tui") {
			...
			await prompter.note([
				...
				controlUiOpened ? "Opened in your browser. Keep that tab to control OpenPaw." : "Copy/paste this URL in a browser on this machine to control OpenPaw.",
				controlUiOpenHint
			].filter(Boolean).join("\n"), "Dashboard ready");
		} else await prompter.note(`When you're ready: ${formatCliCommand("openpaw dashboard --no-open")}`, "Later");
	} else if (opts.skipUi) await prompter.note("Skipping Control UI/TUI prompts.", "Control UI");
	await prompter.note(["Back up your agent workspace.", "Docs: https://docs.openpaw.ai/concepts/agent-workspace"].join("\n"), "Workspace backup");
	await prompter.note("Running agents on your computer is risky — harden your setup: https://docs.openpaw.ai/security", "Security");
	...
	await prompter.note("What now: https://openpaw.ai/showcase (\"What People Are Building\").", "What now");
	await prompter.outro(controlUiOpened ? "🐾 All set! Your terminal just got a whole lot smarter. Dashboard opened — keep that tab handy!" : "🐾 You're all configured! Use the dashboard link above to control OpenPaw. Time to ship something great!");
	return { launchedTui };
}
