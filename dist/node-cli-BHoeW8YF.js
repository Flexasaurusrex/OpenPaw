import { d as resolveIsNixMode, g as resolveStateDir } from "./paths-5liSMgvV.js";
import { B as theme, R as colorize } from "./utils-CLEQvfyB.js";
import { r as resolveAgentConfig } from "./agent-scope-DVw9WyO3.js";
...
import { Y as analyzeArgvCommand, i as loadConfig } from "./config-BrzzL48f.js";
...
import { i as resolveBrowserConfig } from "./server-context-Ce1QVTUf.js";
...
import { n as createBrowserRouteDispatcher, r as getMachineDisplayName, t as withTimeout } from "./with-timeout-DXNXiNQ6.js";
import { n as evaluateShellAllowlist, r as resolveAllowAlwaysPatterns, t as evaluateExecAllowlist } from "./exec-approvals-allowlist-DrS302vF.js";
import { n as resolveExecSafeBinRuntimePolicy } from "./exec-safe-bin-runtime-policy-CGgle7ZQ.js";
import { Zt as loadOrCreateDeviceIdentity, t as GatewayClient } from "./client-8n3vPrA3.js";
...
import { s as resolveGatewayLogPaths } from "./service-C4VX1Mu3.js";
import { a as createCliStatusTextStyles, f as resolveRuntimeStatusColor, g as installDaemonServiceAndEmit, h as createDaemonActionContext, i as runServiceUninstall, m as buildDaemonServiceSnapshot, r as runServiceStop, t as runServiceRestart } from "./lifecycle-core-CvGCh1rn.js";
import { s as formatRuntimeStatus } from "./systemd-hints-k_ESxlcH.js";
...
const execHostFallbackAllowed = process.env.OPENPAW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
async function runNodeHost(opts) {
	...
	const token = process.env.OPENPAW_GATEWAY_TOKEN?.trim() || (isRemoteMode ? cfg.gateway?.remote?.token : cfg.gateway?.auth?.token);
	const password = process.env.OPENPAW_GATEWAY_PASSWORD?.trim() || (isRemoteMode ? cfg.gateway?.remote?.password : cfg.gateway?.auth?.password);
...
async function buildNodeInstallPlan(params) {
	...
	return {
		...
		workingDirectory,
		environment,
		description: formatNodeServiceDescription({ version: environment.OPENPAW_SERVICE_VERSION })
	};
}
...
function registerNodeCli(program) {
	const node = program.command("node").description("Run and manage the headless node host service").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["openpaw node run --host 127.0.0.1 --port 18789", "Run the node host in the foreground."],
		["openpaw node status", "Check node host service status."],
		["openpaw node install", "Install the node host service."],
		["openpaw node restart", "Restart the installed node host service."]
