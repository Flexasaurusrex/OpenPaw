import { t as CONFIG_PATH } from "./paths-5liSMgvV.js";
import { B as theme, R as colorize, S as shortenHomePath, z as isRich$1 } from "./utils-CLEQvfyB.js";
import "./thinking-EAliFiVK.js";
import { St as loadProviderUsageSummary, Tt as resolveUsageProviderId, Yt as describeFailoverError, _r as DEFAULT_OAUTH_WARN_MS, gr as maskApiKey, l as runEmbeddedPiAgent, vr as buildAuthHealthSummary, wt as formatUsageWindowSummary, yr as formatRemainingShort } from "./reply-C7c1Otq4.js";
import { A as resolveAgentModelFallbackValues, M as toAgentModelListLike, O as resolveDefaultAgentWorkspaceDir, i as resolveAgentDir, j as resolveAgentModelPrimaryValue, l as resolveAgentWorkspaceDir, o as resolveAgentExplicitModelPrimary, s as resolveAgentModelFallbacksOverride, u as resolveDefaultAgentId } from "./agent-scope-DVw9WyO3.js";
import { f as defaultRuntime } from "./subsystem-BlU6Lg5x.js";
import "./exec-DkdDR1S0.js";
import { At as upsertAuthProfile, Dt as listProfilesForProvider, It as resolveAuthStorePathForDisplay, J as resolveApiKeyForProvider, Jt as DEFAULT_PROVIDER, K as getCustomProviderApiKey, Lt as resolveOpenPawAgentDir, Mt as ensureAuthProfileStore, Q as getShellEnvAppliedKeys, Ut as resolveAuthProfileDisplayLabel, X as resolveEnvApiKey, Y as resolveAwsSdkEnvVarName, g as resolveModelRefFromString, i as findNormalizedProviderValue, kt as setAuthProfileOrder, l as normalizeProviderId, m as resolveDefaultModelForAgent, p as resolveConfiguredModelRef, pt as resolveAuthProfileOrder, qt as DEFAULT_MODEL, r as buildModelAliasIndex, rt as shouldEnableShellEnvFallback, s as modelKey, u as parseModelRef, vt as resolveProfileUnusableUntilForDisplay } from "./model-selection-DBzxxd2t.js";
...
import { C as parseDurationMs, i as loadConfig } from "./config-BrzzL48f.js";
...
import { t as ensureOpenPawModelsJson } from "./models-config-DtMqhpqZ.js";
import { n as discoverModels, t as discoverAuthStorage } from "./pi-model-discovery-Cojifu4Y.js";
...
import { a as resolveSessionTranscriptPath, c as resolveSessionTranscriptsDirForAgent } from "./paths-UwFQANoB.js";
...
import { r as resolveForwardCompatModel } from "./model-CES2iFyq.js";
import { t as ensurePiAuthJsonFromAuthProfiles } from "./model-C5vbLS4U.js";
...
import { r as withProgressTotals } from "./progress-BrNyn1gy.js";
...
import { f as openUrl } from "./onboard-helpers-Urhlbb7I.js";
...
import { t as createClackPrompter } from "./clack-prompter-7NUHBvOR.js";
...
import { a as isLocalBaseUrl, c as normalizeAlias, d as resolveModelTarget, f as updateConfig, i as formatTokenK, l as resolveKnownAgentId, n as ensureFlagCompatibility, o as loadValidConfigOrThrow, r as formatMs, s as mergePrimaryFallbackConfig, t as applyDefaultModelPrimaryUpdate, u as resolveModelKeysFromEntries } from "./shared-Dv0UevL7.js";
import { d as applyAuthProfileConfig, n as validateAnthropicSetupToken } from "./auth-token-CTISlFQm.js";
import { n as logConfigUpdated } from "./logging-CZH6rk0L.js";
import { a as createVpsAwareOAuthHandlers, c as githubCopilotLoginCommand, i as resolveProviderMatch, n as mergeConfigPatch, o as isRemoteEnvironment, r as pickAuthMethod, s as resolvePluginProviders, t as applyDefaultModel } from "./provider-auth-helpers-D8-7JFsm.js";
...
function resolveRequestedLoginProviderOrThrow(providers, rawProvider) {
	...
	throw new Error(`Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("openpaw plugins list --json")}\`.`);
}
...
async function modelsAuthLoginCommand(opts, runtime) {
	...
	if (providers.length === 0) throw new Error(`No provider plugins found. Install one via \`${formatCliCommand("openpaw plugins install")}\`.`);
...
async function modelsListCommand(opts, runtime) {
	ensureFlagCompatibility(opts);
	const { loadConfig } = await import("./config-BrzzL48f.js").then((n) => n.t);
	const { ensureAuthProfileStore } = await import("./model-selection-DBzxxd2t.js").then((n) => n.ft);
	const cfg = loadConfig();
	...
	if (opts.all) {
		...
		if (!model) {
			const { resolveModel } = await import("./model-C5vbLS4U.js");
...
async function modelsStatusCommand(opts, runtime) {
	...
	if (missingProvidersInUse.length > 0) {
		...
		for (const provider of missingProvidersInUse) {
			const hint = provider === "anthropic" ? `Run \`claude setup-token\`, then \`${formatCliCommand("openpaw models auth setup-token")}\` or \`${formatCliCommand("openpaw configure")}\`.` : `Run \`${formatCliCommand("openpaw configure")}\` or set an API key env var.`;
...
function registerModelsCli(program) {
	const models = program.command("models").description("Model discovery, scanning, and configuration").option("--status-json", "Output JSON (alias for `models status --json`)", false).option("--status-plain", "Plain output (alias for `models status --plain`)", false).option("--agent <id>", "Agent id to inspect (overrides OPENPAW_AGENT_DIR/PI_CODING_AGENT_DIR)").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/models", "docs.openpaw.ai/cli/models")}\n`);
	...
	models.command("status").description("Show configured model state").option("--json", "Output JSON", false).option("--plain", "Plain output", false).option("--check", "Exit non-zero if auth is expiring/expired (1=expired/missing, 2=expiring)", false).option("--probe", "Probe configured provider auth (live)", false).option("--probe-provider <name>", "Only probe a single provider").option("--probe-profile <id>", "Only probe specific auth profile ids (repeat or comma-separated)", (value, previous) => {
		...
		next.push(value);
		return next;
	}).option("--probe-timeout <ms>", "Per-probe timeout in ms").option("--probe-concurrency <n>", "Concurrent probes").option("--probe-max-tokens <n>", "Probe max tokens (best-effort)").option("--agent <id>", "Agent id to inspect (overrides OPENPAW_AGENT_DIR/PI_CODING_AGENT_DIR)").action(async (opts, command) => {
