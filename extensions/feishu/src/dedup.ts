...
import { createDedupeCache, createPersistentDedupe } from "openpaw/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.OPENPAW_STATE_DIR?.trim() || env.PAWBOT_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["openpaw-vitest", String(process.pid)].join("-"));
  }
