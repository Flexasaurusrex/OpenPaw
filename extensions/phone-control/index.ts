...
import type { OpenPawPluginApi, OpenPawPluginService } from "openpaw/plugin-sdk";
...
function normalizeDenyList(cfg: OpenPawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OpenPawPluginApi["config"]): string[] {
...
export default function register(api: OpenPawPluginApi) {
  ...
  const timerService: OpenPawPluginService = {
