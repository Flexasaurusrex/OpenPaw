...
import { approveDevicePairing, listDevicePairing } from "openpaw/plugin-sdk";
...
function resolveGatewayPort(cfg: OpenPawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.OPENPAW_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.PAWBOT_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuth(cfg: OpenPawPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([
      process.env.OPENPAW_GATEWAY_TOKEN,
      process.env.PAWBOT_GATEWAY_TOKEN,
      cfg.gateway?.auth?.token,
    ]) ?? undefined;
  const password =
    pickFirstDefined([
      process.env.OPENPAW_GATEWAY_PASSWORD,
      process.env.PAWBOT_GATEWAY_PASSWORD,
...
async function resolveGatewayUrl(api: OpenPawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the OpenPaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the OpenPaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
