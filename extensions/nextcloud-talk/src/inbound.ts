...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as OpenPawConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as OpenPawConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as OpenPawConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as OpenPawConfig);
